import type { Dictionary } from "./en";

export const tr: Dictionary = {
  meta: {
    label: "Türkçe",
    short: "TR",
    htmlLang: "tr",
  },

  nav: {
    home: "Ana sayfa",
    whatIs: "Golden Visa",
    services: "Hizmetler",
    about: "Hakkımızda",
    whyUs: "Neden biz",
    offices: "Ofisler",
    properties: "Gayrimenkul",
    latvia: "Letonya",
    contact: "İletişim",
  },

  common: {
    bookConsultation: "Danışmanlık randevusu alın",
    talkToUs: "Bir danışmanla görüşün",
    learnMore: "Daha fazla bilgi",
    allServices: "Tüm hizmetler",
    backToServices: "Hizmetlere dön",
    ourServices: "Hizmetlerimiz",
    getInTouch: "İletişime geçin",
    skipToContent: "İçeriğe geç",
    menu: "Menü",
    more: "Daha fazla",
    close: "Kapat",
    language: "Dil",
    theme: "Açık / koyu tema",
    needsClientInput: "Müşteri materyali bekleniyor",
  },

  home: {
    hero: {
      eyebrow: "Golden Visa",
      title: "Avrupa'da oturum",
      titleAccent: "YUNANİSTAN – LETONYA",
      lead: "Avrupa dışından gelen yatırımcılar ve aileleri için Golden Visa alıyor, Yunanistan'da banka hesabı açıyor ve üst segment gayrimenkulleri yönetiyoruz — ilk piyasa araştırmasından anahtarın elinize geçtiği güne kadar.",
      ctaPrimary: "Danışmanlık randevusu alın",
      ctaSecondary: "Golden Visa nedir?",
      note: "Atina · Selanik · Vietnam · Türkiye",
    },
    stats: [
      { value: "2014", label: "Golden Visa alanında çalıştığımız yıl" },
      { value: "4", label: "Üç kıtada ofis" },
      { value: "5", label: "Dosyanızın işlendiği dil" },
      { value: "90", label: "Alım tamamlandıktan sonra onaya kadar geçen gün" },
    ],
    statsNote:
      "Onay süresi, Yunan makamlarının belirlediği uygunluk kriterlerini karşılamanız koşuluyla geçerlidir.",
    whatIs: {
      eyebrow: "Program",
      title: "Aynı zamanda bir yatırım olan oturma izni",
      body: "Yunanistan Golden Visa, Avrupa dışından yatırımcılara ve ailelerine, uygun bir gayrimenkul yatırımı karşılığında Yunanistan'da yaşama ve Schengen bölgesinde serbestçe seyahat etme hakkı verir. Çoğu oturum yolundan farklı olarak yatırılan sermaye bir harç değildir — sahibi olduğunuz, kiraya verebileceğiniz ve satabileceğiniz bir varlıkta kalır.",
      cta: "Program nasıl işliyor",
    },
    services: {
      eyebrow: "Ne yapıyoruz",
      title: "Altı hizmet, tek dosya, tek ekip",
      lead: "Yatırımcıların çoğunun bir izinden fazlasına ihtiyacı var. Zincirin tamamını biz üstleniyoruz — böylece vize, banka, gayrimenkul ve evrak dört ayrı şirkette birbirini işaret etmiyor.",
    },
    process: {
      eyebrow: "Nasıl çalışıyoruz",
      title: "İlk görüşmeden oturma iznine",
      lead: "Baştan sona tek bir muhatap. Her aşamayı yurt dışından tamamlayabilirsiniz.",
      steps: [
        {
          n: "01",
          title: "Danışmanlık",
          body: "Hedeflerinizi, aile durumunuzu ve bütçenizi programın bugünkü koşullarıyla karşılaştırır, uygun olup olmadığınızı açıkça söyleriz.",
        },
        {
          n: "02",
          title: "Gayrimenkul araştırması",
          body: "Araştırma ekibimiz, Yunanistan'ın her büyük şehrindeki ofis ağımıza dayanarak ticari ve konut piyasasından uygun gayrimenkulleri kısa listeye alır.",
        },
        {
          n: "03",
          title: "Bankacılık ve durum tespiti",
          body: "Adınıza Yunanistan'da banka hesabı açarız — siz orada olsanız da olmasanız da — ve tek bir euro hareket etmeden önce gayrimenkulün hukuki kontrollerini yaparız.",
        },
        {
          n: "04",
          title: "Alım ve başvuru",
          body: "Hukuk ve finans ekibimiz alımı tamamlar ve sizin ile aileniz için Golden Visa başvurusunu ek ücret almadan yapar.",
        },
        {
          n: "05",
          title: "İzinden sonra",
          body: "Tadilat, kiralama, vergi ve gayrimenkul yönetimi — siz sonraki adıma karar verirken varlığın kazanmaya devam etmesini sağlarız.",
        },
      ],
    },
    founder: {
      eyebrow: "Yönetim",
      title: "Nikolaos T. Kakkos, M.B.A., C.F.A.",
      role: "Kurucu · N. Kakkos Estate",
      quote: "Yanınızda duruyor ve işiniz tamamlanana kadar sizin için çalışıyoruz.",
      body: "2014'ten bu yana Golden Visa alanında çalışan ve bir ev yönetimi şirketinin sahibi olan Yunan girişimci. New York Adelphi Üniversitesi'nden M.B.A. derecesine ve New York Üniversitesi'nden Finansal Analiz sertifikasına sahiptir.",
      cta: "Ekiple tanışın",
    },
    whyUs: {
      eyebrow: "Neden biz",
      title: "Pazarlamayla değil, kuruluşundan çok uluslu",
      body: "Çok uluslu iş geçmişimiz, profesyonel ihtiyaçlarınızı derinlemesine anlamamızı garanti eder. Zamanınıza saygı duyar, verimli çalışır ve yatırımınıza tamamen sadık kalırız.",
      points: [
        {
          title: "Hukuki destek dâhil",
          body: "Dosyanız üzerinde bir avukat çalışır ve ücretini biz öderiz — sonda ek bir kalem olarak size fatura edilmez.",
        },
        {
          title: "Uçağa binmeniz gerekmez",
          body: "Banka hesabı, alım ve başvuru; dünyanın neresinde olursanız olun tamamlanabilir.",
        },
        {
          title: "Satış konuşması değil, finansal analiz",
          body: "Her gayrimenkul getiri, piyasa koşulları, imar durumu ve yeniden satış açısından değerlendirilir — kendi portföyümüz için değerlendirdiğimiz gibi.",
        },
        {
          title: "Vizeden sonra da buradayız",
          body: "Tadilat, kiralama ve yönetim, izin çıktıktan çok sonra da varlığı üretken tutar.",
        },
      ],
    },
    ctaBand: {
      title: "Zaman önemlidir.",
      body: "Dünyanın neresinde olursanız olun sizin adınıza başvurabilir ve çalışabiliriz. Daha fazla bilgi için arayın veya e-posta gönderin.",
      button: "Başvurunuzu başlatın",
    },
  },

  whatIs: {
    eyebrow: "Program",
    title: "Golden Visa nedir?",
    lead: "Yunanistan dâhil birçok Avrupa ülkesinin sunduğu, ek avantajlar taşıyan bir oturum vizesi.",
    intro: [
      "Golden Visa, Avrupa dışından yatırımcılara ve ailelerine Schengen bölgesinde vizesiz seyahat imkânı ile birlikte ev sahibi ülkede ikamet hakkı tanıyan bir yatırım programıdır.",
      "Her şeyden önce bir yatırım fırsatı olduğu için, yatırımın geri dönüş hızı performans göstergelerinin başında gelir. Programın elverişli koşulları, Yunan ekonomisinin hızla büyüyen endeksleriyle birleştiğinde hem hızlı ve güvenilir bir yatırım hem de ileride daha yüksek fiyattan satış için kazançlı seçenekler sunan umut verici bir zemin oluşturur.",
    ],
    advantagesTitle: "Golden Visa'nın avantajları",
    advantages: [
      "Uzun vadeli oturum hakkı",
      "Yaşama, eğitim görme ve yatırım yapma imkânı",
      "Aile bireylerini dâhil edebilme",
      "İşletmelerde %100 sahiplik",
      "Schengen bölgesinde serbest seyahat",
      "Sağlık ve konaklama hizmetlerinde indirim ve avantajlar",
      "Yatırım getirisi ve sermaye kazancı",
      "Vergi avantajları",
      "Gayrimenkul alımında kısıtlama bulunmaması",
      "Yatırım yapılan gayrimenkulü uzun dönem kiraya verebilme",
      "Belirli bir ikamet süresinden sonra vatandaşlığa başvurabilme",
      "Yunanistan'da kayıtlı bir şirkette hisse tutabilme ve temettü geliri elde edebilme",
    ],
    thresholds: {
      title: "Yatırım eşikleri",
      body: "Uygun tutar, gayrimenkulün nerede olduğuna ve türüne bağlıdır. Aşağıdakiler bugün yürürlükte olan kademelerdir — herhangi bir taahhütte bulunmadan önce hedeflediğiniz bölge için kesin rakamı yazılı olarak teyit ediyoruz.",
      cta: "Hedef bölgeniz için bize sorun",
      sourceNote:
        "5100/2024 sayılı Kanun, 5 Nisan 2024'ten beri yürürlükte; 22 Nisan 2026 tarihli 1/2026 sayılı Genelge ile açıklığa kavuşturulmuştur. Eşikler mevzuatla belirlenir ve değişebilir.",
      verified: "23 Ağustos 2026 tarihinde doğrulandı",
      tiers: [
        {
          amount: "250.000 €",
          where: "Yalnızca iki özel durum, ülkenin her yerinde",
          note: "Ticari kullanımdan konuta dönüştürülmüş ve dönüşümü başvurudan önce tamamlanmış gayrimenkul; veya ilk yenilemeden önce tamamen restore edilen tescilli yapı. Gayrimenkul başına bir kez geçerlidir.",
        },
        {
          amount: "400.000 €",
          where: "Yunanistan'ın diğer tüm bölgeleri",
          note: "Mora Yarımadası'nın büyük kısmı, iç kesimler, Selanik dışındaki kuzey Yunanistan ve küçük adalar.",
        },
        {
          amount: "800.000 €",
          where: "Attika · Selanik bölgesi · Mikonos · Santorini · nüfusu 3.100'ün üzerindeki adalar",
          note: "Atina ve tanınmış adaların çoğu dâhil, talebin yüksek olduğu bölgeler.",
        },
      ],
      sizeTitle: "Asgari büyüklük: 120 m²",
      sizeBody:
        "Yatırım, ana alanı en az 120 m² olan TEK bir gayrimenkul olmalıdır. Otopark ve depo sayılmaz. Bu kural boş arsalara ve 250.000 € dönüşüm/restorasyon yollarına uygulanmaz.",
      startupTitle: "2026'da yeni: inovasyon yolu",
      startupBody:
        "2026'dan itibaren program, ulusal Elevate platformuna kayıtlı Yunan start-up ve spin-off şirketlerine 250.000 €'dan başlayan yatırımları da kabul ediyor. Bireysel yatırımcı için bir izin, yatırımın kilit yöneticileri de desteklediği durumlarda üç ize kadar. Bunun size gayrimenkulden daha uygun olup olmadığını bize sorun.",
    },

    rules: {
      title: "İzin size ne verir, ne vermez",
      lead: "Yatırımcıların en sık yanlış bildiği noktalar. Gayrimenkul seçmeden önce okumaya değer.",
      items: [
        {
          title: "Beş yıl, süresiz yenilenir",
          body: "İzin beş yıl geçerlidir ve gayrimenkul sizin mülkiyetinizde kaldığı sürece her defasında aynı süreyle yenilenir. Yenileme, gayrimenkul beyanı veya kadastro kaydıyla kanıtlanır.",
        },
        {
          title: "Asgari kalış şartı yok",
          body: "Yunanistan'da yaşamak zorunda değilsiniz. Ülke dışında geçirilen süre yenilemeyi reddetmek için gerekçe değildir. Amacınız oturum değil vatandaşlıksa kurallar farklıdır — bize sorun.",
        },
        {
          title: "Tek başvuruda üç kuşak",
          body: "Eşiniz veya kayıtlı partneriniz, 21 yaşın altındaki bekâr çocuklarınız (24'e kadar uzatılabilir) ve hem sizin hem eşinizin anne-babası ile büyükanne-büyükbabası iznini sizinkinden alır.",
        },
        {
          title: "Kısa dönem kiralama yasak",
          body: "Golden Visa için tutulan gayrimenkul, tatil kiralama platformları üzerinden kısa dönem kiralanamaz ve alt kiraya verilemez. Cezası iznin iptali ve 50.000 € para cezasıdır. Uzun dönem kiralama serbesttir; konaklamanın ötesinde hizmet sunan turizm işletmelerine kiralamak da dâhil.",
        },
        {
          title: "Bu izin çalışma izni değildir",
          body: "İstihdama erişim hakkı vermez. Yunan şirketi kurabilir, hisse tutabilir ve temettü alabilirsiniz; ancak bu izne dayanarak işe giremezsiniz.",
        },
      ],
    },

    faq: {
      title: "Yatırımcıların gerçekten sorduğu sorular",
      items: [
        {
          q: "Masrafları karşılamak için Airbnb'de kiraya verebilir miyim?",
          a: "Hayır. Golden Visa kapsamındaki gayrimenkulün tatil kiralama platformlarında kısa dönem kiralanması ve alt kiraya verilmesi yasaktır. İhlal, izninize ve 50.000 € para cezasına mal olur. Uzun dönem kiralama serbesttir ve normalde bizim düzenlediğimiz yol budur.",
        },
        {
          q: "Yunanistan'a taşınmam gerekir mi?",
          a: "Hayır. Asgari kalış şartı yoktur ve ülke dışında bulunmak yenilemeyi reddetmek için gerekçe değildir. Müşterilerimizin çoğu, izin hazır olana kadar Yunanistan'a ayak basmadan tüm süreci tamamlar.",
        },
        {
          q: "Eşiği tutturmak için iki ucuz gayrimenkul alabilir miyim?",
          a: "Hayır. Yatırım, bulunduğu bölgenin eşiğini karşılayan TEK bir gayrimenkul olmalıdır; 800.000 € ve 400.000 € kademelerinde ana alanı en az 120 m² olmalıdır.",
        },
        {
          q: "Başvuruya kimleri dâhil edebilirim?",
          a: "Eşinizi veya kayıtlı partnerinizi, 21 yaşın altındaki bekâr çocuklarınızı — 24'e kadar uzatılabilir — ve hem sizin hem eşinizin anne-babası ile büyükanne-büyükbabasını.",
        },
        {
          q: "İzin Yunan vatandaşlığına götürür mü?",
          a: "Otomatik olarak değil. Golden Visa bir oturma iznidir. Vatandaşlığa kabul ayrı bir kanunla düzenlenir ve Yunanistan'da gerçek anlamda uzun süreli ikamet gerektirir; Golden Visa sizi buna zorunlu tutmaz. Var olmayan bir yol ima etmek yerine durumunuzun neye izin verdiğini açıkça söyleriz.",
        },
        {
          q: "Bu izinle Yunanistan'da çalışabilir miyim?",
          a: "Hayır. İstihdama erişim hakkı vermez. Şirket kurabilir, hisse tutabilir ve temettü alabilirsiniz.",
        },
        {
          q: "Ne kadar sürer?",
          a: "Alımınız tamamlanıp dosyanız düzene girdiğinde, makamlar izni işleme koyarken yasal olarak ikamet etmenizi sağlayan belge düzenlenir. Kendi durumunuz için gerçekçi bir takvimi yazılı veririz — herkese uyan tek bir rakam söylemeyiz.",
        },
      ],
    },
    eligibilityTitle: "Kimler için",
    eligibility: [
      "Yunan gayrimenkulüne yatırım yapan AB dışı vatandaşlar",
      "Aynı başvuruya dâhil edilen eşler, çocuklar ve bakmakla yükümlü olunan ebeveynler",
      "Tam zamanlı taşınmadan Schengen hareketliliği isteyen yatırımcılar",
      "Gayrimenkulü boş bırakmak yerine kiraya vermeyi düşünen alıcılar",
    ],
    disclaimer:
      "Bu sayfa program hakkında genel bilgi içerir; hukuki danışmanlık veya kişiye özel yatırım tavsiyesi değildir. Uygunluk koşulları, eşikler ve süreler Yunan makamlarınca belirlenir ve değişebilir.",
  },

  services: {
    eyebrow: "Ne yapıyoruz",
    title: "Hizmetler",
    lead: "İlk piyasa araştırmasından varlığın günlük yönetimine kadar yatırımın tamamını kapsayan altı hizmet.",
    items: {
      "golden-visa-acquisition": {
        name: "Golden Visa başvurusu",
        tagline: "Başvuru, hukuk ve finans ekibimiz tarafından ek ücret alınmadan yürütülür.",
        body: [
          "Sürecin tamamını müşterilerimiz için mümkün olduğunca sorunsuz ve zahmetsiz hâle getirmeye kararlıyız. Uzman hukuk ve finans ekibimiz, sizin ve aileniz için Golden Visa başvuru sürecini ek ücret almadan derhal başlatır.",
          "Uygunluk kriterlerini karşıladığınız sürece, gayrimenkul alımınızı tamamlamanızdan itibaren 90 gün içinde Golden Visa onayınızı almayı bekleyebilirsiniz.",
        ],
        highlight: {
          label: "%100 ücretsiz",
          text: "Başvuru işlemleri ve avukat masrafı bize aittir — bizim tarafımızdan ayrı bir başvuru ücreti alınmaz.",
        },
        bullets: [
          "Taahhütte bulunmadan önce uygunluk değerlendirmesi",
          "Tüm aile için belge hazırlığı",
          "Yunan makamlarına başvuru ve takip",
          "Biyometri ve izin teslimi sizin adınıza koordine edilir",
          "Yenileme hatırlatmaları ve destek",
        ],
        note: "Zaman önemlidir. Dünyanın neresinde olursanız olun sizin adınıza başvurabilir ve çalışabiliriz.",
      },
      "bank-account-acquisition": {
        name: "Banka hesabı açılışı",
        tagline: "Adınıza Yunanistan'da banka hesabı; orada olsanız da olmasanız da.",
        body: [
          "Yunanistan'da Avrupa Merkez Bankası tarafından lisanslanmış dört büyük özel banka bulunmaktadır. Sizin adınıza başvuru yapıp hesap açabiliriz.",
          "Zaman önemli olduğundan, süreci Yunanistan'da bulunmanızla veya bulunmanıza gerek kalmadan tamamlayabiliriz.",
        ],
        highlight: {
          label: "Uzaktan yapılabilir",
          text: "Hesap, siz kendi ülkenizdeyken açılabilir.",
        },
        bullets: [
          "Profilinize ve oturum durumunuza göre banka seçimi",
          "Uyum ve fon kaynağı dosyasının ilk seferde doğru hazırlanması",
          "Aracı üzerinden değil, doğrudan kendi adınıza hesap",
          "İnternet bankacılığının kurulması ve anlatılması",
        ],
        note: "",
      },
      "real-estate-advisory": {
        name: "Gayrimenkul danışmanlığı",
        tagline: "On yılı aşkın araştırma, analiz, planlama ve strateji.",
        body: [
          "On yılı aşkın deneyimimizle, bir yatırım portföyünün gayrimenkul bölümünü değerlendirirken, satın alırken, satarken, geliştirirken, iyileştirirken ve finanse ederken araştırma, analiz, planlama, strateji ve yönetim katkısı sağlıyoruz.",
          "Gayrimenkul yatırım faaliyetlerinize yön vermek ve emlakçıların işlem odaklı çalışmasını tamamlamak için gerektiğinde bir finansal danışmanla da birlikte çalışırız.",
        ],
        highlight: {
          label: "Aracılık değil, danışmanlık",
          text: "Belirli bir işlemi kapatmak için değil, varlık hakkında doğru öngörüde bulunmak için çalışırız.",
        },
        bullets: [
          "Satışa sunulan gayrimenkullerin araştırılması",
          "Piyasa koşullarının değerlendirilmesi",
          "İmar gerekliliklerinin kontrolü",
          "Önerilen alımların değerlendirilmesi",
          "Sermaye tahsis stratejilerinin oluşturulması",
          "Tadilat projelerinin fizibilitesinin incelenmesi",
          "Operasyonel iyileştirme stratejileri geliştirilmesi",
          "Kiracı memnuniyetini artırma yollarının incelenmesi",
          "Kiralama ile satın alma kararlarının dengelenmesi",
        ],
        note: "",
      },
      "real-estate-research": {
        name: "Gayrimenkul araştırması",
        tagline: "Kendi araştırma ekibimiz ve her büyük Yunan şehrinde ofis ağı.",
        body: [
          "Araştırma ekibimiz Yunanistan'ın ticari ve konut gayrimenkul piyasalarına odaklanır. Her büyük şehirdeki emlak ofisi ağıyla desteklenerek size en uygun fırsatı bulurlar.",
        ],
        highlight: {
          label: "Her iki piyasa",
          text: "Ticari ve konut; aynı titizlikle araştırılır.",
        },
        bullets: [
          "Bütçeniz ve hedefiniz etrafında kurulan kısa listeler",
          "Mahalle düzeyinde fiyat ve getiri verileri",
          "Yerel ofis ağı üzerinden piyasa dışı fırsatlar",
          "Herhangi bir yeri görmeden önce karşılaştırmalı analiz",
        ],
        note: "",
      },
      renovation: {
        name: "Tadilat hizmetleri",
        tagline: "Evleri yuvaya dönüştürmede onlarca yıllık deneyim.",
        body: [
          "Komple tadilat, yenileme, ev iyileştirme, peyzaj, iç mimari ve daha fazlasında onlarca yıllık deneyimimiz var. Hayalinizi alır, gerçeğe dönüştürürüz.",
          "Mutfak tadilatından komple ev yenilemesine kadar her projeye uzmanlığımızı ve işçiliğimizi katıyoruz. Profesyonel müteahhit ve tasarımcı ekibimiz; zamanında teslim, maliyet etkinliği ve en kaliteli malzemelerle olağanüstü sonuçlar sunmakla gurur duyar.",
        ],
        highlight: {
          label: "Zamanında, bütçesinde",
          text: "İşe başlamadan önce maliyetlendirilir ve aynı ekip sonuna kadar sorumludur.",
        },
        bullets: [
          "Komple tadilat ve yenileme",
          "Ev iyileştirme ve onarım",
          "Peyzaj düzenlemesi",
          "İç mimari",
          "Mutfak ve banyo tadilatı",
          "Uzun dönem kiralamaya yönelik donatım",
        ],
        note: "",
      },
      "property-management": {
        name: "Gayrimenkul yönetimi",
        tagline: "Avrupa'nın en iyi üst segment gayrimenkul portföylerinden biri.",
        body: [
          "Gayrimenkul yönetmek bir sanattır: deneyim, uzmanlık ve hizmet kalitesine tam bağlılık gerektirir. Kooperatif ve site tipi konutları düzenleyen çok sayıda karmaşık mevzuata uyum, doğru kayıt tutma prosedürleri ve organizasyon becerisi ister.",
          "Bugüne kadar Avrupa'nın en iyi üst segment gayrimenkul portföylerinden birini başarıyla yönetiyoruz. Müşterilerimize yüksek gelir sağlarken yatırımlarına değer katıyoruz.",
        ],
        highlight: {
          label: "Gelir ve değer",
          text: "Varlık hem kazanmalı hem değerlenmeli — ikisi de yönetilir, şansa bırakılmaz.",
        },
        bullets: [
          "Kiracı bulma, değerlendirme ve sözleşmeler",
          "Kira tahsilatı ve raporlama",
          "Mevzuata uyum ve kayıt tutma",
          "Bakım, onarım ve denetimler",
          "Faturalar, vergiler ve aidatlar",
          "Uzun dönem kiralama, turizm işletmelerine kiralama dâhil",
        ],
        note: "",
      },
    },
  },

  about: {
    eyebrow: "Hakkımızda",
    title: "NIBELC Group hakkında",
    lead: "2014'ten bu yana Avrupa dışı yatırımcılarla çalışan bir Yunan gayrimenkul ve yatırım yoluyla oturum firması.",
    groupAlt: "Sn. Nikolaos T. Kakkos, Sn. Stella Nguyen ve Sn. Tony Phan",
    intro: [
      "Genellikle ayrı şirketlerde duran üç işin kesişiminde çalışıyoruz: oturma izni, gayrimenkul ve sonrasında varlığı üretken tutan her şey. Üçünü tek ekipte tutmamız, sorularınızı başka yere yönlendirmek yerine takvim taahhüdü verebilmemizi sağlıyor.",
      "Müşterilerimiz Avrupa Birliği dışındaki yatırımcılar ve aileler. Çoğu, izin hazır olana kadar Yunanistan'a ayak basmadan tüm süreci tamamlıyor.",
    ],
    founder: {
      name: "Nikolaos T. Kakkos, M.B.A., C.F.A.",
      role: "Kurucu, N. Kakkos Estate",
      body: "Nikolaos Kakkos, 2014'ten bu yana Golden Visa alanında çalışan ve bir ev yönetimi şirketinin sahibi olan başarılı bir Yunan girişimcidir. New York Adelphi Üniversitesi'nden M.B.A. derecesine sahiptir ve New York Üniversitesi tarafından Finansal Analiz alanında sertifikalandırılmıştır.",
      credentials: [
        "M.B.A., Adelphi Üniversitesi, New York",
        "Finansal Analiz sertifikası, New York Üniversitesi",
        "2014'ten bu yana Golden Visa alanında",
        "Bir ev yönetimi şirketinin kurucusu",
      ],
    },
    /** Ba người khách xác nhận. Bằng cấp giữ nguyên văn khách gửi. */
    team: [
      {
        name: "Nikolaos T. Kakkos",
        honorific: "M.B.A., C.F.A.",
        role: "Kurucu · N. Kakkos Estate, Atina",
        credentials: [
          "M.B.A., Adelphi University, New York",
          "Certified in Financial Analysis, New York University",
        ],
      },
      {
        name: "Tony Phan",
        honorific: "LL.M., M.Sc.",
        role: "NIBELC Group",
        credentials: [
          "Hukuk Yüksek Lisansı",
          "Gayrimenkul Yatırımı Yüksek Lisansı (Londra, Birleşik Krallık)",
        ],
      },
      {
        name: "Stella Nguyen",
        honorific: "M.Sc.",
        role: "NIBELC Group",
        credentials: [
          "Ekonomi Yüksek Lisansı (Buckingham, Birleşik Krallık)",
        ],
      },
    ],
    galleryTitle: "Sahada",
    galleryLead: "Avrupa ve Vietnam’da diplomatik temsilciliklerle görüşmeler, sektör fuarları ve şirket etkinlikleri.",
    galleryAlt: "NIBELC Group sahada",
    teamTitle: "Ekip",
    teamLead: "Hukuk, finans, araştırma ve tadilat uzmanları aynı dosya üzerinde çalışır. Aşağıda işi yürüten üç kişi var.",
    legalTitle: "Hukuki ve noterlik desteği",
    legalBody:
      "Dosyanız üzerinde bir avukat çalışır ve ücretini biz öderiz. İşlemin gerektirdiği durumlarda ayrıca bir noter devreye girer. Sizin adınıza kimin, hangi sıfatla hareket ettiği önceden bildirilir.",
    portraitPending:
      "Geçici arma. Müşteri fotoğraf gönderdiğinde Sn. Nikolaos'un portresiyle değiştirilecek.",
    placeholderTeam: "Ekip fotoğrafları müşteri tarafından verilecek. İsimler ve dereceler teyit edilmiştir.",
  },

  whyUs: {
    eyebrow: "Neden biz",
    title: "Neden bizimle çalışmalısınız",
    lead: "Çok uluslu iş geçmişimiz, profesyonel ihtiyaçlarınızı derinlemesine anlamamızı garanti eder.",
    body: [
      "Zamanınıza saygı duyar, verimli çalışır ve yatırımınıza tamamen sadık kalırız. Yanınızda durur ve işiniz tamamlanana kadar sizin için çalışırız.",
    ],
    points: [
      {
        title: "Tek ekip, tek dosya",
        body: "Vize, banka, gayrimenkul, tadilat ve yönetim aynı kişiler tarafından yürütülür. Hiçbir şey iki tedarikçi arasında kaybolmaz.",
      },
      {
        title: "Hukuki masraf bizde",
        body: "Golden Visa başvurunuzda çalışan avukatın ücretini biz öderiz; sonda faturanıza eklenmez.",
      },
      {
        title: "Varsayılan olarak uzaktan",
        body: "Banka hesabı, alım ve başvuru; dünyanın neresinde olursanız olun tamamlanabilir.",
      },
      {
        title: "Yatırım disiplini",
        body: "Getiri, piyasa koşulları, imar durumu ve yeniden satış potansiyeli, gayrimenkul size gösterilmeden önce değerlendirilir.",
      },
      {
        title: "Beş dil",
        body: "Dosyanız İngilizce, Vietnamca, Yunanca, Letonca ve Türkçe olarak, bu dilleri ana dili gibi konuşan kişilerce yürütülür.",
      },
      {
        title: "Dört pazarda varlık",
        body: "Atina, Selanik, Vietnam ve Türkiye — size en yakın ofis gerçek bir ofistir, posta kutusu değil.",
      },
    ],
  },

  offices: {
    eyebrow: "Neredeyiz",
    title: "Ofisler",
    lead: "Üç kıtada altı ofis. Dosyanız size uygun dilde ve saat diliminde yürütülür.",
    items: [
      {
        slug: "athens",
        city: "Atina",
        country: "Yunanistan",
        address: "1 Pigasou Street, Kifisia 14564, Yunanistan",
        phone: "+30 697 300 8000",
        role: "Merkez ofis · Golden Visa, hukuk ve finans ekibi",
        confirmed: true,
      },
      {
        slug: "thessaloniki",
        city: "Selanik",
        country: "Yunanistan",
        address: "Filippou 28, Perea 57019, Yunanistan",
        phone: "+49 152 0696 8888",
        role: "NIBELC ofisi · Kuzey Yunanistan",
        confirmed: true,
      },
      {
        slug: "budapest",
        city: "Budapeşte",
        country: "Macaristan",
        address: "József krt. 45, 2em.6B, 1085 Budapeşte, Macaristan",
        phone: "",
        role: "NIBELC Manpower Kft · Orta Avrupa",
        confirmed: true,
      },
      {
        slug: "germany",
        city: "Berlin",
        country: "Almanya",
        address: "Dietrichstraße 16, 16356 Ahrensfelde, Almanya",
        phone: "",
        role: "NIBELC Germany GmbH · Ahrensfelde",
        confirmed: true,
      },
      {
        slug: "hanoi",
        city: "Hanoi",
        country: "Vietnam",
        address: "4. kat, Rainbow Binası, Lot B – CQ1, Van Quan, Ha Dong, Hanoi, Vietnam",
        phone: "",
        role: "Vietnam pazarı · müşteri danışmanlığı",
        confirmed: true,
      },
      {
        slug: "turkiye",
        city: "Türkiye",
        country: "Türkiye",
        address: "",
        phone: "",
        role: "Türkiye pazarı · müşteri danışmanlığı",
        confirmed: false,
      },
    ],
    pendingLabel: "Adres teyit edilecek",
    pendingNote: "Bu arada lütfen Atina merkez ofisimizle iletişime geçin.",
  },

  contact: {
    eyebrow: "İletişim",
    title: "Bir danışmanla görüşün",
    lead: "Nerede olduğunuzu, neyi hedeflediğinizi ve yaklaşık bütçenizi bize yazın. Programın size uyup uymadığını ve şu anda neler gerektirdiğini açıkça söyleyelim.",
    form: {
      name: "Ad soyad",
      email: "E-posta",
      phone: "Telefon / WhatsApp",
      country: "İkamet ülkesi",
      interest: "İlgilendiğim konu",
      interestOptions: [
        "Golden Visa başvurusu",
        "Yunanistan'da banka hesabı",
        "Gayrimenkul danışmanlığı",
        "Gayrimenkul araştırması",
        "Tadilat",
        "Gayrimenkul yönetimi",
        "Başka bir konu",
      ],
      budget: "Yaklaşık bütçe",
      budgetOptions: [
        "Henüz karar vermedim",
        "250.000 € altı",
        "250.000 € – 400.000 €",
        "400.000 € – 800.000 €",
        "800.000 € üzeri",
      ],
      message: "Mesajınız",
      consent:
        "Bilgilerimin, Gizlilik Politikası'nda açıklandığı şekilde bu talebi yanıtlamak için kullanılmasını kabul ediyorum.",
      submit: "Talebi gönder",
      sending: "Gönderiliyor…",
      successTitle: "Teşekkürler — talebiniz bize ulaştı.",
      successBody: "Bir danışmanımız genellikle bir iş günü içinde size şahsen dönecektir.",
      errorTitle: "Mesajınız gönderilemedi.",
      errorBody: "Lütfen tekrar deneyin veya doğrudan Atina ofisimize e-posta gönderin.",
      required: "Zorunlu",
      invalidEmail: "Lütfen geçerli bir e-posta adresi girin",
      consentRequired: "Devam etmek için gizlilik bildirimini kabul edin",
    },
    directTitle: "Doğrudan iletişim",
    headOffice: "Merkez ofis",
    offices: "Ofislerimiz",
    directions: "Yol tarifi",
    hours: "Pazartesi – Cuma, 09:00 – 18:00 (Doğu Avrupa Saati)",
    responseNote: "İngilizce, Vietnamca, Yunanca, Letonca ve Türkçe yanıt veriyoruz.",
  },

  legal: {
    updated: "Son güncelleme",
    privacy: {
      title: "Gizlilik Politikası",
      lead: "Bize gönderdiğiniz kişisel verileri nasıl işlediğimiz.",
      sections: [
        {
          title: "Biz kimiz",
          body: "Filippou 28, Perea 57019, Yunanistan adresindeki NIBELC GROUP – N. KAKKOS ESTATE, bu web sitesi aracılığıyla toplanan kişisel verilerin veri sorumlusudur.",
        },
        {
          title: "Neleri topluyoruz",
          body: "Yalnızca iletişim formuna yazdıklarınızı: adınız, e-posta adresiniz, telefon numaranız, ikamet ülkeniz, ilgilendiğiniz hizmet, yaklaşık bütçeniz ve mesajınız. Kişisel veri satın almıyor ve üçüncü taraflardan toplamıyoruz.",
        },
        {
          title: "Neden kullanıyoruz",
          body: "Yalnızca talebinizi yanıtlamak ve müşterimiz olmanız hâlinde bizden aldığınız hizmetleri yürütmek için. Verilerinizi satmıyoruz ve önce size sormadan ilgisiz pazarlama amacıyla kullanmıyoruz.",
        },
        {
          title: "Ne kadar süre saklıyoruz",
          body: "Sözleşmeye dönüşmeyen talepler 24 ay sonra silinir. Müşteri dosyaları Yunan mevzuatının gerektirdiği süre boyunca saklanır.",
        },
        {
          title: "Haklarınız",
          body: "GDPR kapsamında verilerinize erişim, düzeltme, silme, işlemenin kısıtlanması veya taşınabilir biçimde bir kopya talep edebilir ve işlemeye itiraz edebilirsiniz. Atina ofisimize yazın, bir ay içinde yanıt verelim. Ayrıca ulusal veri koruma otoritenize şikâyette bulunma hakkınız vardır.",
        },
        {
          title: "Çerezler",
          body: "Siz açıkça kabul etmedikçe bu sitede reklam veya analiz çerezi kullanılmaz. Zorunlu çerezler sitenin çalışmasını sağlar ve kapatılamaz.",
        },
      ],
    },
    terms: {
      title: "Kullanım Koşulları",
      lead: "Bu web sitesinin sunulma esasları.",
      sections: [
        {
          title: "Sitenin amacı",
          body: "Bu web sitesi NIBELC GROUP – N. KAKKOS ESTATE'in hizmetlerini tanıtır. Bir teklif, fiyat teklifi veya sözleşme değil, hizmetlere bir giriştir.",
        },
        {
          title: "Doğruluk",
          body: "Bilgileri güncel tutmaya özen gösteriyoruz; ancak göçmenlik kuralları, yatırım eşikleri ve işlem süreleri kamu otoritelerince belirlenir ve önceden haber verilmeksizin değişir. Buradaki hiçbir bilgiye, kendi durumunuz için bizden yazılı teyit almadan güvenilmemelidir.",
        },
        {
          title: "Fikrî mülkiyet",
          body: "Bu sitedeki metin, görsel ve tasarım NIBELC GROUP – N. KAKKOS ESTATE'e aittir veya izinle kullanılmaktadır. Yazılı onay olmadan çoğaltılamaz.",
        },
        {
          title: "Dış bağlantılar",
          body: "Resmî kurumlara veya üçüncü taraflara verilen bağlantılar yalnızca referans amaçlıdır. İçeriklerinden sorumlu değiliz.",
        },
        {
          title: "Uygulanacak hukuk",
          body: "Bu koşullar Yunan hukukuna tabidir.",
        },
      ],
    },
    disclaimer: {
      title: "Yasal Uyarı",
      lead: "Bu web sitesi nedir, ne değildir.",
      sections: [
        {
          title: "Hukuki veya yatırım tavsiyesi değildir",
          body: "Bu web sitesinin içeriği genel bilgidir. Hukuki danışmanlık, vergi danışmanlığı veya kişiye özel yatırım tavsiyesi değildir ve bireysel koşullarınızı dikkate almaz.",
        },
        {
          title: "Sonuç garantisi yoktur",
          body: "Herhangi bir oturma izninin, vizenin veya vatandaşlığın verileceğini garanti etmiyoruz, edemeyiz. Bu kararlar tamamen yetkili makamlara aittir. İşlem sürelerine ilişkin ifadeler yalnızca başvuranın o tarihte yürürlükteki uygunluk kriterlerini karşılaması hâlinde geçerlidir.",
        },
        {
          title: "Getiri garantisi yoktur",
          body: "Gayrimenkul değerleri ve kira gelirleri artabileceği gibi düşebilir de. Yunan gayrimenkul piyasasının geçmiş performansı geleceğe dair bir vaat değildir ve yatırımınızın getirisi garanti edilmez.",
        },
        {
          title: "Eşikler değişir",
          body: "Yunanistan Golden Visa yatırım eşikleri güncellenmiştir ve bölgeye ile gayrimenkul türüne bağlıdır. Size bildirilen her rakam, işleminizin yapıldığı tarihte yürürlükte olan kurallara göre yazılı olarak teyit edilir.",
        },
      ],
    },
  },

  cookie: {
    title: "Çerezler",
    body: "Bu siteyi çalıştırmak için yalnızca zorunlu çerezleri kullanıyoruz. Sitenin nasıl kullanıldığını anlamak için isteğe bağlı analiz çerezleri de yerleştirmek istiyoruz. Hiçbir işlevi kaybetmeden reddedebilirsiniz.",
    accept: "İsteğe bağlıyı kabul et",
    decline: "İsteğe bağlıyı reddet",
    link: "Gizlilik Politikası",
  },

  footer: {
    tagline: "Yunanistan'da Golden Visa, gayrimenkul danışmanlığı ve mülk yönetimi.",
    company: "Şirket",
    servicesCol: "Hizmetler",
    legalCol: "Yasal",
    contactCol: "İletişim",
    rights: "Tüm hakları saklıdır.",
    regNote: "Şirket tescil bilgileri müşteri tarafından sağlanacaktır.",
    disclaimerShort:
      "Bu sitedeki bilgiler geneldir ve hukuki veya yatırım tavsiyesi teşkil etmez. Oturma izinleri Yunan makamlarınca verilir.",
  },

  /**
   * Số liệu lấy từ tài liệu khách gửi: CHƯƠNG TRÌNH VISA ĐẦU TƯ VÀ ĐỊNH CƯ
   * LATVIA.pdf (10 trang). Diện dùng ở đây là GÓP VỐN DOANH NGHIỆP — đúng
   * diện mà Luật Nhập cư mới giữ lại, không phải diện bất động sản hay gửi
   * tiền ngân hàng đang bị đề xuất bỏ. `lawBody` nói rõ chỗ này.
   */
  latvia: {
    eyebrow: "Letonya",
    title: "Şirket yatırımıyla Letonya'da oturum",
    lead: "Program, yatırımcıya ve ailesine bir Letonya şirketine doğrudan yatırım yoluyla Avrupa'da oturum hakkı verir; koşullar sağlanırsa daimi oturuma ve vatandaşlığa giden bir yol da açar.",
    photoAlt: [
      "Riga'da Kara Kafalar Evi meydanı",
      "Gün batımında Daugava kıyısında Riga",
      "Riga eski şehir çatıları",
      "Riga'da modern cam mimari",
      "Riga silüeti",
    ],
    countryTitle: "Ülke",
    country: [
      "In northern Europe on the Baltic Sea — the gateway between the Nordic countries and central and eastern Europe.",
      "A population of about two million. The capital is Riga.",
      "Joined the European Union in 2004 and the Schengen area in 2007.",
      "Visa-free travel, residence and study across the 29 Schengen member states.",
      "A major logistics hub, well placed for cross-border trade.",
      "Modern technical infrastructure, with internet speeds among the fastest in the world.",
    ],
    benefitTitle: "Size ne kazandırır",
    residencyTitle: "Oturum",
    residency: [
      "Legal right of residence in Latvia",
      "The right to live, study and invest in Latvia",
      "Freedom of movement across the Schengen area",
      "The ability to bring family members",
      "A route to permanent residence and citizenship if the conditions are met",
    ],
    investTitle: "Yatırım",
    invest: [
      "You may own and run a business in Latvia",
      "Access to the European market through that business",
      "You may hold shares and receive dividends from a Latvian-registered company",
      "Return on the investment and capital growth",
    ],
    condTitle: "Kimler başvurabilir",
    basicTitle: "Temel koşullar",
    basic: [
      "Over 18 and not a national of the EU, EEA or Switzerland",
      "A clean criminal record, with no serious convictions",
      "A valid passport and health insurance valid in the Schengen area",
      "Proof of the lawful source of funds, under anti-money-laundering rules",
      "The financial capacity to make the investment",
    ],
    amountTitle: "Gereken yatırım",
    amount: [
      "At least € 50,000 into the share capital of a commercial company.",
      "Once the first permit is approved, a further € 10,000 is paid into the state budget.",
      "The usual routes: incorporate a new company, invest in a local business, or buy shares in a trading company.",
    ],
    familyTitle: "Kapsanan aile üyeleri",
    family: [
      "Spouse and children under 18",
      "Children over 18 who are still studying and dependent",
    ],
    costTitle: "Tahmini toplam maliyet",
    costCols: ["Kalem", "Tutar (€)"],
    costs: [
      { item: "Share capital investment", amount: "55,000" },
      { item: "Residence permit fee", amount: "5,000" },
      { item: "Other costs — state fees, insurance, medical (estimate)", amount: "500" },
      { item: "Registration of a Latvian residential address (annual)", amount: "1,500" },
      { item: "Payment into the state budget, after approval", amount: "10,000" },
    ],
    costTotalLabel: "Toplam",
    costTotal: "72,000",
    costNotes: [
      "Translation costs may arise if documents are not in English, German or French.",
      "Some smaller costs vary case by case.",
      "The balance to be held in the Latvian account is around € 84,000, covering the investment and the required minimum living costs.",
    ],
    processTitle: "Süreç",
    process: [
      { title: "Prepare the file in your home country", body: "Passport valid for at least a year, 4×5 photographs on a white background, a criminal record certificate, consular-legalised documents, and proof of funds." },
      { title: "Prepare the funds", body: "Put in place the balance required in the Latvian account, covering the investment and the required minimum living costs." },
      { title: "File in Latvia", body: "Sign the power of attorney, open a Latvian bank account, transfer the funds to it, and file with the migration authority (OCMA)." },
      { title: "Wait for the decision", body: "Fees by speed: 30 days € 160 per person, 10 days € 280, 5 days € 560. This is the time to a decision, which is not always the final outcome." },
      { title: "Receive the outcome", body: "The file may be approved, refused, or returned for further documents — the last is common. It can take up to four months, and up to twelve in unusual cases." },
      { title: "Make the investment", body: "Incorporate or invest in the Latvian company, complete the legal and operational steps, and keep the conditions met for renewal." },
    ],
    lawTitle: "Zamanlamayla ilgili bilinmesi gereken",
    lawBody: "Latvia's Immigration Law is being revised. The text passed by parliament on 11 June 2026 removes the real estate route and the bank deposit route, but KEEPS the company investment route — the route this programme uses. The President returned the law for a second review on 19 June and parliament took it up again on 20 August; as at the date checked the law is not in force, so applications are processed under the current rules. We confirm in writing what applies to your case before you transfer any money.",
    verified: "Rakamlar NIBELC program dosyasından · hukuki durum 28 Ağustos 2026'da kontrol edildi",
    cta: "Letonya hakkında bize sorun",
    note: "This page is general information about the programme, not legal or investment advice. Conditions, amounts and processing times are set by the Latvian authorities and can change.",
  },

  properties: {
    eyebrow: "Gayrimenkul",
    title: "Peristeri, Atina'da dört daire",
    lead: "Tamamen yenilenmiş dört daire. Aşağıda geliştiricinin verdiği dosyanın tamamı var: tanıtım, satış fiyatı, tahmini kira, kat planları ve iç mekân görselleri.",
    linkLabel: "Dört dairenin dosyasını görün",
    backLabel: "Ana sayfaya dön",
    priceLabel: "Satış fiyatı",
    rentLabel: "Uzun dönem kira",
    bedsLabel: "Yatak odası",
    bathsLabel: "Banyo",
    sizeLabel: "Alan",
    sizePending: "Teyit ediliyor",
    typeLabel: "Tür",
    typeValue: "Daire",
    capacityLabel: "Tasarım kapasitesi",
    introTitle: "Tanıtım",
    intro: "Daireler Peristeri, Atina'da yer alıyor. Gerçek yaşam için tasarlandı — derli toplu, kullanışlı; küçük bir aile, tek kişi ya da uzun dönem ikamet için uygun. İç mekân modern ve nötr, kişiselleştirmesi kolay.",
    planTitle: "Kat planı",
    layoutTitle: "Daire kurgusu",
    galleryTitle: "İç mekân görselleri",
    galleryNote: "Görseller geliştirici tarafından verildi ve dört dairenin tamamı için ortak. Kat planları ise her daireye özel.",
    cta: "Ayrıntılı dosyayı isteyin",
    note: "Fiyatlar ve durum 26 Ağustos 2026 itibarıyladır, değişebilir. Bir yatırımın Golden Visa için yeterli olup olmadığı bölgeye ve gayrimenkul türüne bağlıdır — her daire için bunu siz karar vermeden önce yazılı olarak teyit ediyoruz.",
    eligibilityTitle: "Değerlendirmeden önce bilinmesi gerekenler",
    eligibility: [
      "Peristeri, Attika bölgesindedir. 2024'ten bu yana Attika'da gayrimenkul yolu için eşik 800.000 €'dur. 250.000–270.000 € fiyatıyla bu daireler Golden Visa için otomatik olarak YETERLİ DEĞİLDİR; yalnızca ticari yapının konuta dönüştürülmesi veya tescilli yapının restorasyonu kapsamında yeterli olur.",
      "Geliştiricinin dosyasında alan “XXX m²” olarak yazıyor — gerçek rakam henüz yok. Kanun 400.000 € ve 800.000 € eşiklerinde en az 120 m² istiyor; dolayısıyla bu bilgi karardan önce şarttır.",
      "Golden Visa için kullanılan gayrimenkul Airbnb tarzı kısa dönem kiralanamaz: ihlali oturum izninin iptaline ve 50.000 €'ya kadar para cezasına yol açar. Bu daireler yalnızca uzun dönem kiralanmalıdır.",
    ],
    commitTitle: "Her daireyle birlikte verilen taahhütler",
    commitments: [
      "Daire %100 yeni olarak yenilendi",
      "Ev sahibi teslim aldıktan sonra iyi bir bedelle geri kiralama",
      "Sürdürülebilir işletme ve dairenin durumunun korunması",
    ],
    items: {
      "apt-1": {
        name: "Daire 1",
        capacity: "yaklaşık 10 kişi",
        layout: "Plan ortak konaklama modeline göre kurgulanmış; merkezî yatma alanında alanı en verimli kullanmak için iki yanda ranzalar var. Ortak yaşam alanı ve dolaplar ön tarafta, açık mutfak ve uzun yemek masasıyla doğrudan bağlantılı. Tuvalet ve banyo grubu tek bir tarafta toplanmış ve ayrılmış, böylece aynı anda birkaç kişi kullanabiliyor. Bütünde plan ferah ve bölgeleri net.",
      },
      "apt-2": {
        name: "Daire 2",
        capacity: "en fazla 8 kişi",
        layout: "Daire grup konaklama modeline göre tasarlanmış. Yatma alanı ranzalarla merkezîleştirilmiş ve iki adet ebeveyn banyosu içeriyor. Açık mutfağın ortak yaşam alanıyla birleşmesi daireyi ferah tutuyor, yemek yapmayı ve bir araya gelmeyi kolaylaştırıyor. Binanın ortak merdivenine doğrudan bağlanan bir geçiş var; bu da dolaşımı ve yönetimi kolaylaştırıyor.",
      },
      "apt-3": {
        name: "Daire 3",
        capacity: "en fazla 8 kişi",
        layout: "Birleştirilmiş bir daire (Apartment 03 & 04): tek bir büyük daireye dönüştürülmüş, kalabalık aileler için uygun. İçinde iki ayrı yatma alanı, iki ortak yaşam alanı, iki açık mutfak, eksiksiz dolap sistemi ve her bölgenin mahremiyetini koruyacak biçimde yerleştirilmiş çok sayıda banyo ile tuvalet var. İki girişi (ana ve tali) bulunuyor; plan esnek ve ihtiyaca göre kolayca uyarlanabiliyor.",
      },
      "apt-4": {
        name: "Daire 4",
        capacity: "en fazla 10 kişi",
        layout: "Daire grup konaklama modeline göre tasarlanmış. İçinde ranzalı merkezî bir yatma alanı, çok sayıda banyo ile ayrı tuvaletler, geniş bir ortak yaşam alanıyla birleşen açık mutfak ve kullanışlı bir depolama bölümü var. Plan verimli ve aynı anda birkaç kişinin kullanımına elverişli.",
      },
    },
  },

  assistant: {
    open: "Asistana sorun",
    close: "Sohbeti kapat",
    title: "Golden Visa asistanı",
    subtitle: "Yayımlanmış bilgilerimizden yanıt, 7/24",
    greeting:
      "Merhaba. Yunanistan Golden Visa, hizmetlerimiz veya ofislerimiz hakkında sorabilirsiniz. Kendi dosyanızla ilgili her şey için sizi bir danışmana yönlendiririm.",
    placeholder: "Sorunuzu yazın…",
    send: "Gönder",
    thinking: "Yazıyor…",
    suggestions: [
      "Güncel yatırım eşikleri nedir?",
      "Gayrimenkulü Airbnb'de kiralayabilir miyim?",
      "Yunanistan'da yaşamam gerekir mi?",
      "Başvuruma kimleri dâhil edebilirim?",
    ],
    offlineNote: "— yanıt, yayımlanmış SSS bölümümüzden alındı.",
    noAnswer:
      "Yayımlanmış bilgilerimizde bu yok. Bir danışman doğru yanıtı verebilir — iletişim sayfasını kullanın veya Atina ofisini arayın.",
    humanCta: "Bir insanla görüşün",
    disclaimer:
      "Otomatik asistan. Yalnızca genel bilgi — hukuki veya yatırım tavsiyesi değildir. Lütfen pasaport, banka veya belge bilgilerinizi buraya yazmayın.",
    errorMsg: "Bir sorun oluştu. Lütfen tekrar deneyin veya doğrudan bize ulaşın.",
  },

  notFound: {
    title: "Sayfa bulunamadı",
    body: "Aradığınız sayfa mevcut değil veya taşınmış.",
    cta: "Ana sayfaya dön",
  },
};
