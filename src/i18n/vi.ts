import type { Dictionary } from "./en";

export const vi: Dictionary = {
  meta: {
    label: "Tiếng Việt",
    short: "VI",
    htmlLang: "vi",
  },

  nav: {
    home: "Trang chủ",
    whatIs: "Thẻ vàng",
    services: "Dịch vụ",
    about: "Về chúng tôi",
    whyUs: "Vì sao chọn chúng tôi",
    offices: "Văn phòng",
    properties: "Bất động sản",
    latvia: "Latvia",
    contact: "Liên hệ",
  },

  common: {
    bookConsultation: "Đặt lịch tư vấn",
    talkToUs: "Nói chuyện với chuyên viên",
    learnMore: "Tìm hiểu thêm",
    allServices: "Tất cả dịch vụ",
    backToServices: "Quay lại dịch vụ",
    ourServices: "Dịch vụ của chúng tôi",
    getInTouch: "Liên hệ",
    skipToContent: "Bỏ qua, vào nội dung chính",
    menu: "Menu",
    more: "Thêm",
    close: "Đóng",
    language: "Ngôn ngữ",
    theme: "Đổi nền sáng / tối",
    needsClientInput: "Đang chờ khách hàng cung cấp",
  },

  home: {
    hero: {
      eyebrow: "Golden Visa",
      title: "Định cư châu Âu",
      titleAccent: "HY LẠP – LATVIA",
      lead: "Chúng tôi làm thẻ vàng, mở tài khoản ngân hàng Hy Lạp và quản lý bất động sản cao cấp cho nhà đầu tư ngoài châu Âu cùng gia đình — từ bản nghiên cứu thị trường đầu tiên đến lúc chìa khoá nằm trong tay quý vị.",
      ctaPrimary: "Đặt lịch tư vấn",
      ctaSecondary: "Thẻ vàng là gì?",
      note: "Athens · Thessaloniki · Việt Nam · Thổ Nhĩ Kỳ",
    },
    stats: [
      { value: "2014", label: "Làm dịch vụ thẻ vàng từ năm" },
      { value: "4", label: "Văn phòng trên ba châu lục" },
      { value: "5", label: "Ngôn ngữ xử lý hồ sơ của quý vị" },
      { value: "90", label: "Ngày có kết quả sau khi hoàn tất mua nhà" },
    ],
    statsNote:
      "Thời gian xét duyệt áp dụng với điều kiện quý vị đáp ứng đủ tiêu chí do cơ quan Hy Lạp quy định.",
    whatIs: {
      eyebrow: "Về chương trình",
      title: "Giấy phép cư trú đồng thời là một khoản đầu tư",
      body: "Thẻ vàng Hy Lạp cho nhà đầu tư ngoài châu Âu và gia đình quyền sinh sống tại Hy Lạp và đi lại tự do trong khối Schengen, đổi lại một khoản đầu tư bất động sản đủ điều kiện. Khác với hầu hết các con đường định cư khác, số vốn này không phải là lệ phí — nó nằm trong một tài sản quý vị sở hữu, có thể cho thuê và có thể bán lại.",
      cta: "Chương trình vận hành thế nào",
    },
    services: {
      eyebrow: "Chúng tôi làm gì",
      title: "Sáu dịch vụ, một bộ hồ sơ, một đội ngũ",
      lead: "Nhà đầu tư thường cần nhiều hơn một tấm thẻ. Chúng tôi lo trọn chuỗi — để visa, ngân hàng, bất động sản và giấy tờ không nằm ở bốn công ty rồi đùn đẩy cho nhau.",
    },
    process: {
      eyebrow: "Cách chúng tôi làm việc",
      title: "Từ cuộc gọi đầu tiên đến thẻ cư trú",
      lead: "Một đầu mối duy nhất xuyên suốt. Quý vị có thể hoàn tất mọi bước từ nước ngoài.",
      steps: [
        {
          n: "01",
          title: "Tư vấn",
          body: "Chúng tôi đối chiếu mục tiêu, hoàn cảnh gia đình và ngân sách của quý vị với quy định hiện hành, rồi nói thẳng quý vị có đủ điều kiện hay không.",
        },
        {
          n: "02",
          title: "Tìm bất động sản",
          body: "Đội nghiên cứu lọc danh sách bất động sản đủ điều kiện từ thị trường thương mại và nhà ở, dựa trên mạng lưới văn phòng ở mọi thành phố lớn của Hy Lạp.",
        },
        {
          n: "03",
          title: "Ngân hàng và thẩm định",
          body: "Chúng tôi mở tài khoản ngân hàng Hy Lạp đứng tên quý vị — có mặt hay không có mặt đều được — và kiểm tra pháp lý bất động sản trước khi một đồng euro nào được chuyển đi.",
        },
        {
          n: "04",
          title: "Mua và nộp hồ sơ",
          body: "Đội pháp lý và tài chính hoàn tất giao dịch mua rồi nộp hồ sơ thẻ vàng cho quý vị và gia đình, không tính thêm chi phí.",
        },
        {
          n: "05",
          title: "Sau khi có thẻ",
          body: "Cải tạo, cho thuê, thuế và quản lý vận hành — chúng tôi giữ cho tài sản sinh lời trong lúc quý vị tính bước tiếp theo.",
        },
      ],
    },
    founder: {
      eyebrow: "Người dẫn dắt",
      title: "Nikolaos T. Kakkos, M.B.A., C.F.A.",
      role: "Nhà sáng lập · N. Kakkos Estate",
      quote:
        "Chúng tôi đứng cạnh quý vị và làm việc cho quý vị cho đến khi công việc hoàn tất.",
      body: "Doanh nhân Hy Lạp, làm dịch vụ thẻ vàng và sở hữu một công ty quản lý nhà ở từ năm 2014. Ông có bằng M.B.A. của Đại học Adelphi (New York) và chứng chỉ Phân tích Tài chính do Đại học New York cấp.",
      cta: "Gặp đội ngũ",
    },
    whyUs: {
      eyebrow: "Vì sao chọn chúng tôi",
      title: "Đa quốc gia từ cách vận hành, không phải từ lời quảng cáo",
      body: "Nền tảng kinh doanh đa quốc gia của chúng tôi bảo đảm sự thấu hiểu sâu sắc nhu cầu công việc của quý vị. Chúng tôi tôn trọng thời gian của quý vị, làm việc hiệu quả và tuyệt đối trung thành với khoản đầu tư của quý vị.",
      points: [
        {
          title: "Đã bao gồm hỗ trợ pháp lý",
          body: "Có luật sư làm hồ sơ cho quý vị và chi phí do chúng tôi trả — không phải một dòng phụ phí xuất hiện ở cuối bảng kê.",
        },
        {
          title: "Quý vị không cần bay sang",
          body: "Tài khoản ngân hàng, giao dịch mua và hồ sơ đều có thể hoàn tất khi quý vị đang ở bất cứ đâu trên thế giới.",
        },
        {
          title: "Phân tích tài chính, không phải lời chào hàng",
          body: "Mỗi bất động sản đều được đánh giá về lợi suất, tình hình thị trường, quy hoạch và khả năng bán lại — đúng như cách chúng tôi đánh giá cho danh mục của chính mình.",
        },
        {
          title: "Chúng tôi ở lại sau khi có visa",
          body: "Cải tạo, cho thuê và quản lý giữ cho tài sản tiếp tục sinh lời rất lâu sau khi thẻ được cấp.",
        },
      ],
    },
    ctaBand: {
      title: "Thời gian là quan trọng.",
      body: "Chúng tôi có thể nộp hồ sơ và làm việc thay quý vị trong khi quý vị ở bất cứ đâu trên thế giới. Gọi hoặc gửi email để biết thêm chi tiết.",
      button: "Bắt đầu hồ sơ",
    },
  },

  whatIs: {
    eyebrow: "Về chương trình",
    title: "Thẻ vàng là gì?",
    lead: "Một loại thị thực cư trú kèm nhiều quyền lợi cộng thêm, do một số quốc gia châu Âu cấp, trong đó có Hy Lạp.",
    intro: [
      "Thẻ vàng là một chương trình đầu tư cho phép nhà đầu tư ngoài châu Âu và gia đình đi lại miễn thị thực trong khối Schengen, đồng thời có quyền cư trú tại nước sở tại.",
      "Trước hết đây là một cơ hội đầu tư, nên tốc độ hoàn vốn nằm ở nhóm chỉ số quan trọng nhất. Điều kiện thuận lợi của chương trình cộng với các chỉ số tăng trưởng nhanh của kinh tế Hy Lạp tạo ra bối cảnh hứa hẹn cho một khoản đầu tư nhanh và đáng tin cậy, cũng như những lựa chọn bán lại giá cao hơn trong tương lai.",
    ],
    advantagesTitle: "Lợi ích của thẻ vàng",
    advantages: [
      "Quyền cư trú dài hạn",
      "Được sinh sống, học tập và đầu tư",
      "Được bảo lãnh người thân trong gia đình",
      "Sở hữu 100% doanh nghiệp",
      "Tự do đi lại trong khối Schengen",
      "Ưu đãi về y tế và dịch vụ lưu trú",
      "Lợi nhuận đầu tư và lãi vốn",
      "Ưu đãi về thuế",
      "Không hạn chế việc mua bất động sản",
      "Được cho thuê dài hạn chính bất động sản đã đầu tư",
      "Đủ điều kiện xin quốc tịch sau một thời gian cư trú",
      "Được nắm giữ cổ phần và nhận cổ tức từ công ty đăng ký tại Hy Lạp",
    ],
    thresholds: {
      title: "Mức đầu tư tối thiểu",
      body: "Số tiền đủ điều kiện phụ thuộc vào bất động sản nằm ở đâu và thuộc loại nào. Dưới đây là các mức đang có hiệu lực — chúng tôi xác nhận bằng văn bản con số chính xác cho đúng khu vực quý vị nhắm tới, trước khi quý vị cam kết bất cứ điều gì.",
      cta: "Hỏi mức áp dụng cho khu vực của tôi",
      sourceNote:
        "Luật 5100/2024, hiệu lực từ 05/04/2024, được làm rõ bởi Thông tư 1/2026 ngày 22/04/2026. Mức đầu tư do luật quy định và có thể thay đổi.",
      verified: "Kiểm chứng ngày 23/08/2026",
      tiers: [
        {
          amount: "250.000 €",
          where: "Chỉ hai trường hợp đặc biệt, ở bất kỳ đâu",
          note: "Bất động sản chuyển đổi từ thương mại sang nhà ở, việc chuyển đổi phải hoàn tất trước khi nộp hồ sơ; hoặc công trình được xếp hạng di sản, trùng tu xong trước lần gia hạn đầu tiên. Mỗi bất động sản chỉ áp dụng một lần.",
        },
        {
          amount: "400.000 €",
          where: "Mọi khu vực còn lại của Hy Lạp",
          note: "Phần lớn bán đảo Peloponnese, vùng đất liền, bắc Hy Lạp ngoài Thessaloniki, và các đảo nhỏ.",
        },
        {
          amount: "800.000 €",
          where: "Attica · vùng Thessaloniki · Mykonos · Santorini · các đảo trên 3.100 dân",
          note: "Những vùng nhu cầu cao, gồm Athens và hầu hết các đảo nổi tiếng.",
        },
      ],
      sizeTitle: "Diện tích tối thiểu: 120 m²",
      sizeBody:
        "Phải là MỘT bất động sản duy nhất, diện tích chính từ 120 m² trở lên. Chỗ để xe và kho không được tính. Quy định này không áp dụng cho đất trống, và không áp dụng cho hai trường hợp 250.000 €.",
      startupTitle: "Mới từ 2026: hướng đầu tư đổi mới sáng tạo",
      startupBody:
        "Từ năm 2026, chương trình chấp nhận thêm hình thức đầu tư từ 250.000 € vào các start-up và spin-off Hy Lạp đăng ký trên nền tảng quốc gia Elevate. Một giấy phép cho nhà đầu tư cá nhân, tối đa ba giấy phép nếu khoản đầu tư còn hỗ trợ nhân sự chủ chốt. Hỏi chúng tôi xem hướng này có hợp với quý vị hơn bất động sản không.",
    },

    rules: {
      title: "Thẻ cho quý vị quyền gì và KHÔNG cho quyền gì",
      lead: "Những điểm nhà đầu tư hay hiểu sai nhất. Nên đọc trước khi chọn bất động sản.",
      items: [
        {
          title: "Năm năm, gia hạn không giới hạn",
          body: "Thẻ có thời hạn năm năm và được gia hạn từng đợt năm năm, chừng nào quý vị còn sở hữu bất động sản. Gia hạn chứng minh bằng tờ khai tài sản hoặc trích lục địa chính.",
        },
        {
          title: "Không bắt buộc cư trú",
          body: "Quý vị không phải sang sống ở Hy Lạp. Thời gian ở ngoài Hy Lạp không phải là lý do từ chối gia hạn. Nếu mục tiêu của quý vị là quốc tịch chứ không phải thẻ cư trú thì quy định khác hẳn — hỏi chúng tôi.",
        },
        {
          title: "Ba thế hệ trong một hồ sơ",
          body: "Vợ/chồng hoặc bạn đời đăng ký, con chưa lập gia đình dưới 21 tuổi (có thể kéo dài tới 24), cùng cha mẹ và ông bà của cả hai vợ chồng đều hưởng quyền từ hồ sơ của quý vị.",
        },
        {
          title: "CẤM cho thuê ngắn hạn",
          body: "Bất động sản dùng để xin thẻ vàng KHÔNG được cho thuê ngắn hạn qua các nền tảng kiểu Airbnb, và không được cho thuê lại. Vi phạm thì bị thu hồi thẻ và phạt 50.000 €. Cho thuê dài hạn thì được, kể cả cho doanh nghiệp du lịch có cung cấp dịch vụ ngoài chỗ ở.",
        },
        {
          title: "Thẻ này không phải giấy phép lao động",
          body: "Thẻ không cho quyền đi làm thuê. Quý vị được lập công ty tại Hy Lạp, nắm cổ phần và nhận cổ tức, nhưng không được nhận việc làm dựa trên tấm thẻ này.",
        },
      ],
    },

    faq: {
      title: "Những câu nhà đầu tư thật sự hỏi",
      items: [
        {
          q: "Tôi cho thuê Airbnb để bù chi phí được không?",
          a: "Không. Bất động sản dùng xin thẻ vàng bị cấm cho thuê ngắn hạn qua các nền tảng nghỉ dưỡng, và cấm cho thuê lại. Vi phạm thì mất thẻ và bị phạt 50.000 €. Cho thuê dài hạn thì được, và đó là cách chúng tôi thường thu xếp.",
        },
        {
          q: "Tôi có phải chuyển sang Hy Lạp sống không?",
          a: "Không. Không có yêu cầu cư trú tối thiểu, và việc vắng mặt khỏi Hy Lạp không phải lý do từ chối gia hạn. Phần lớn khách của chúng tôi hoàn tất toàn bộ quy trình mà chưa đặt chân tới Hy Lạp cho đến khi thẻ sẵn sàng.",
        },
        {
          q: "Tôi mua hai căn rẻ hơn cho đủ mức được không?",
          a: "Không. Khoản đầu tư phải là MỘT bất động sản duy nhất đạt mức của khu vực đó, và tối thiểu 120 m² diện tích chính đối với hai mức 800.000 € và 400.000 €.",
        },
        {
          q: "Tôi được đưa ai vào hồ sơ?",
          a: "Vợ/chồng hoặc bạn đời đăng ký, con chưa lập gia đình dưới 21 tuổi — có thể kéo dài tới 24 — cùng cha mẹ và ông bà của cả hai vợ chồng.",
        },
        {
          q: "Có thẻ này rồi thì được quốc tịch Hy Lạp không?",
          a: "Không tự động. Thẻ vàng là giấy phép cư trú. Việc nhập quốc tịch do luật khác điều chỉnh và đòi hỏi cư trú thực sự lâu dài tại Hy Lạp — điều mà thẻ vàng không bắt buộc quý vị phải làm. Chúng tôi nói thẳng trường hợp của quý vị cho phép tới đâu, chứ không vẽ ra một con đường không có thật.",
        },
        {
          q: "Tôi được đi làm ở Hy Lạp không?",
          a: "Không. Thẻ không cho quyền tiếp cận việc làm. Quý vị được lập công ty, nắm cổ phần và nhận cổ tức.",
        },
        {
          q: "Mất bao lâu?",
          a: "Sau khi hoàn tất giao dịch mua và hồ sơ đầy đủ, quý vị được cấp giấy tờ cho phép cư trú hợp pháp trong lúc cơ quan chức năng xử lý thẻ. Chúng tôi đưa mốc thời gian thực tế cho chính trường hợp của quý vị bằng văn bản — không đưa một con số chung áp cho tất cả mọi người.",
        },
      ],
    },
    eligibilityTitle: "Dành cho ai",
    eligibility: [
      "Công dân ngoài EU đầu tư vào bất động sản Hy Lạp",
      "Vợ/chồng, con cái và cha mẹ phụ thuộc được đưa vào cùng một hồ sơ",
      "Nhà đầu tư muốn đi lại trong Schengen mà không phải chuyển hẳn sang sống",
      "Người mua có ý định cho thuê thay vì để bất động sản trống",
    ],
    disclaimer:
      "Trang này cung cấp thông tin chung về chương trình, không phải tư vấn pháp lý hay tư vấn đầu tư cá nhân hoá. Điều kiện, mức đầu tư và thời gian xử lý do cơ quan chức năng Hy Lạp quy định và có thể thay đổi.",
  },

  services: {
    eyebrow: "Chúng tôi làm gì",
    title: "Dịch vụ",
    lead: "Sáu dịch vụ bao trọn khoản đầu tư, từ bản nghiên cứu thị trường đầu tiên đến việc quản lý tài sản hằng ngày.",
    items: {
      "golden-visa-acquisition": {
        name: "Làm thẻ vàng",
        tagline: "Hồ sơ do đội pháp lý và tài chính của chúng tôi thực hiện, không tính thêm chi phí.",
        body: [
          "Chúng tôi cam kết làm cho toàn bộ quy trình diễn ra suôn sẻ và nhẹ nhàng nhất có thể cho khách hàng. Đội ngũ chuyên gia pháp lý và tài chính của chúng tôi sẽ khởi động ngay hồ sơ thẻ vàng cho quý vị và gia đình mà không tính thêm chi phí.",
          "Với điều kiện quý vị đáp ứng đủ tiêu chí, quý vị có thể kỳ vọng nhận được kết quả thẻ vàng trong vòng 90 ngày kể từ khi hoàn tất giao dịch mua bất động sản.",
        ],
        highlight: {
          label: "Miễn phí 100%",
          text: "Phần việc hồ sơ và chi phí luật sư do chúng tôi lo — về phía chúng tôi không có khoản phí hồ sơ riêng nào.",
        },
        bullets: [
          "Thẩm định điều kiện trước khi quý vị cam kết",
          "Chuẩn bị giấy tờ cho cả gia đình",
          "Nộp và theo hồ sơ với cơ quan chức năng Hy Lạp",
          "Sắp xếp lấy sinh trắc học và nhận thẻ giúp quý vị",
          "Nhắc hạn và hỗ trợ gia hạn",
        ],
        note: "Thời gian là quan trọng. Chúng tôi có thể nộp hồ sơ và làm việc thay quý vị trong khi quý vị ở bất cứ đâu trên thế giới.",
      },
      "bank-account-acquisition": {
        name: "Mở tài khoản ngân hàng",
        tagline: "Tài khoản ngân hàng Hy Lạp đứng tên quý vị, có mặt hay không có mặt đều được.",
        body: [
          "Hy Lạp có bốn ngân hàng tư nhân lớn được Ngân hàng Trung ương châu Âu cấp phép. Chúng tôi có thể thay mặt quý vị làm thủ tục và mở tài khoản đứng tên quý vị.",
          "Vì thời gian là quan trọng, chúng tôi có thể hoàn tất quy trình dù quý vị có mặt tại Hy Lạp hay không.",
        ],
        highlight: {
          label: "Làm từ xa được",
          text: "Tài khoản có thể mở trong lúc quý vị vẫn đang ở trong nước.",
        },
        bullets: [
          "Chọn ngân hàng theo hồ sơ và tình trạng cư trú của quý vị",
          "Hồ sơ tuân thủ và chứng minh nguồn tiền làm đúng ngay từ lần đầu",
          "Tài khoản đứng tên chính quý vị, không qua trung gian",
          "Thiết lập và hướng dẫn sử dụng ngân hàng trực tuyến",
        ],
        note: "",
      },
      "real-estate-advisory": {
        name: "Tư vấn bất động sản",
        tagline: "Hơn mười năm nghiên cứu, phân tích, lập kế hoạch và chiến lược.",
        body: [
          "Với kinh nghiệm hơn mười năm, chúng tôi cung cấp nghiên cứu, phân tích, lập kế hoạch, chiến lược và ý kiến quản trị khi định giá, mua, bán, phát triển, cải thiện và thu xếp vốn cho phần bất động sản trong danh mục đầu tư.",
          "Chúng tôi thậm chí có thể phối hợp với một chuyên gia tư vấn tài chính để định hướng hoạt động đầu tư bất động sản của quý vị, bổ sung cho phần việc thiên về giao dịch của các môi giới và đại lý.",
        ],
        highlight: {
          label: "Tư vấn, không phải môi giới",
          text: "Chúng tôi được trả tiền để nhận định đúng về tài sản, không phải để chốt bằng được một giao dịch cụ thể.",
        },
        bullets: [
          "Nghiên cứu các bất động sản đang chào bán",
          "Đánh giá tình hình thị trường",
          "Kiểm tra yêu cầu quy hoạch",
          "Thẩm định các thương vụ được đề xuất",
          "Xây dựng chiến lược phân bổ vốn",
          "Nghiên cứu tính khả thi của các dự án cải tạo",
          "Xây dựng chiến lược cải thiện vận hành",
          "Nghiên cứu cách nâng mức hài lòng của người thuê",
          "Cân nhắc giữa phương án thuê và mua",
        ],
        note: "",
      },
      "real-estate-research": {
        name: "Nghiên cứu bất động sản",
        tagline: "Đội nghiên cứu riêng và mạng lưới văn phòng ở mọi thành phố lớn của Hy Lạp.",
        body: [
          "Đội nghiên cứu chuyên trách của chúng tôi tập trung vào thị trường bất động sản thương mại và nhà ở tại Hy Lạp. Với mạng lưới văn phòng bất động sản ở mọi thành phố lớn, họ tìm ra thương vụ phù hợp nhất với quý vị.",
        ],
        highlight: {
          label: "Cả hai thị trường",
          text: "Thương mại và nhà ở, được nghiên cứu với cùng một mức độ kỹ lưỡng.",
        },
        bullets: [
          "Danh sách rút gọn xây quanh ngân sách và mục tiêu của quý vị",
          "Dữ liệu giá và lợi suất tới từng khu phố",
          "Cơ hội ngoài thị trường qua mạng lưới văn phòng địa phương",
          "Phân tích so sánh trước khi quý vị đi xem bất cứ căn nào",
        ],
        note: "",
      },
      renovation: {
        name: "Dịch vụ cải tạo",
        tagline: "Hàng chục năm kinh nghiệm biến một căn nhà thành một tổ ấm.",
        body: [
          "Chúng tôi có hàng chục năm kinh nghiệm trong cải tạo toàn diện, sửa sang, nâng cấp nhà ở, cảnh quan sân vườn, thiết kế nội thất và hơn thế nữa. Chúng tôi nhận hình dung của quý vị và biến nó thành hiện thực.",
          "Từ cải tạo bếp đến làm mới toàn bộ căn nhà, chúng tôi đặt chuyên môn và tay nghề vào từng dự án. Đội nhà thầu và nhà thiết kế của chúng tôi tự hào về việc đúng hẹn, tối ưu chi phí và cho ra kết quả xuất sắc, với vật tư chất lượng cao nhất.",
        ],
        highlight: {
          label: "Đúng hẹn, đúng ngân sách",
          text: "Bóc tách chi phí trước khi khởi công, và vẫn là đội đó chịu trách nhiệm đến cuối.",
        },
        bullets: [
          "Cải tạo và sửa sang toàn diện",
          "Nâng cấp và sửa chữa nhà ở",
          "Cảnh quan sân vườn",
          "Thiết kế nội thất",
          "Cải tạo bếp và phòng tắm",
          "Hoàn thiện để cho thuê dài hạn",
        ],
        note: "",
      },
      "property-management": {
        name: "Quản lý bất động sản",
        tagline: "Một trong những danh mục bất động sản cao cấp tốt nhất châu Âu.",
        body: [
          "Quản lý bất động sản là một nghệ thuật: nó đòi hỏi kinh nghiệm, chuyên môn và sự tận tâm tuyệt đối với chất lượng dịch vụ. Việc tuân thủ vô số quy định phức tạp về nhà ở chung cư và hợp tác xã nhà ở đòi hỏi quy trình lưu trữ chính xác và năng lực tổ chức.",
          "Đến nay chúng tôi đang quản lý thành công một trong những danh mục bất động sản cao cấp tốt nhất châu Âu. Chúng tôi mang lại doanh thu cao cho khách hàng đồng thời gia tăng giá trị cho khoản đầu tư của họ.",
        ],
        highlight: {
          label: "Doanh thu và giá trị",
          text: "Tài sản phải vừa sinh lời vừa tăng giá — cả hai đều được quản lý, không phó mặc may rủi.",
        },
        bullets: [
          "Tìm, sàng lọc người thuê và làm hợp đồng",
          "Thu tiền thuê và báo cáo",
          "Tuân thủ quy định và lưu trữ hồ sơ",
          "Bảo trì, sửa chữa và kiểm tra định kỳ",
          "Điện nước, thuế và phí toà nhà",
          "Cho thuê dài hạn, kể cả cho doanh nghiệp du lịch",
        ],
        note: "",
      },
    },
  },

  about: {
    eyebrow: "Về chúng tôi",
    title: "Về NIBELC Group",
    lead: "Công ty bất động sản và định cư qua đầu tư của Hy Lạp, làm việc với nhà đầu tư ngoài châu Âu từ năm 2014.",
    groupAlt: "Ông Nikolaos T. Kakkos, bà Stella Nguyen và ông Tony Phan",
    intro: [
      "Chúng tôi làm ở điểm giao của ba việc mà bình thường nằm ở ba công ty khác nhau: giấy phép cư trú, bất động sản, và tất cả những gì giữ cho tài sản sinh lời về sau. Chính việc giữ cả ba trong một đội ngũ cho phép chúng tôi cam kết về tiến độ thay vì chuyển tiếp câu hỏi của quý vị đi nơi khác.",
      "Khách hàng của chúng tôi là các nhà đầu tư và gia đình ngoài Liên minh châu Âu. Phần lớn hoàn tất toàn bộ quy trình mà chưa đặt chân đến Hy Lạp cho tới khi thẻ đã sẵn sàng.",
    ],
    founder: {
      name: "Nikolaos T. Kakkos, M.B.A., C.F.A.",
      role: "Nhà sáng lập · N. Kakkos Estate",
      body: "Nikolaos Kakkos là một doanh nhân Hy Lạp thành đạt, làm dịch vụ thẻ vàng và sở hữu một công ty quản lý nhà ở từ năm 2014. Ông có bằng M.B.A. của Đại học Adelphi (New York) và chứng chỉ Phân tích Tài chính do Đại học New York cấp.",
      credentials: [
        "M.B.A., Đại học Adelphi, New York",
        "Chứng chỉ Phân tích Tài chính, Đại học New York",
        "Làm dịch vụ thẻ vàng từ năm 2014",
        "Nhà sáng lập một công ty quản lý nhà ở",
      ],
    },
    /** Ba người khách xác nhận. Bằng cấp giữ nguyên văn khách gửi. */
    team: [
      {
        name: "Nikolaos T. Kakkos",
        honorific: "M.B.A., C.F.A.",
        role: "Nhà sáng lập · N. Kakkos Estate, Athens",
        credentials: [
          "M.B.A., Adelphi University, New York",
          "Certified in Financial Analysis, New York University",
        ],
      },
      {
        name: "Tony Phan",
        honorific: "LL.M., M.Sc.",
        role: "NIBELC Group",
        credentials: [
          "Thạc sĩ Luật",
          "Thạc sĩ Đầu tư Bất động sản (London, Anh)",
        ],
      },
      {
        name: "Stella Nguyen",
        honorific: "M.Sc.",
        role: "NIBELC Group",
        credentials: [
          "Thạc sĩ Kinh tế (Buckingham, Anh)",
        ],
      },
    ],
    galleryTitle: "Hoạt động",
    galleryLead: "Làm việc với các cơ quan ngoại giao, hội chợ ngành và sự kiện công ty tại châu Âu và Việt Nam.",
    galleryAlt: "Hoạt động của NIBELC Group",
    teamTitle: "Đội ngũ",
    teamLead: "Chuyên gia pháp lý, tài chính, nghiên cứu và cải tạo cùng làm trên một bộ hồ sơ. Dưới đây là ba người dẫn dắt công việc.",
    legalTitle: "Hỗ trợ luật sư và công chứng",
    legalBody:
      "Có luật sư làm hồ sơ cho quý vị và chi phí do chúng tôi trả. Khi giao dịch đòi hỏi, chúng tôi mời thêm công chứng viên. Quý vị luôn được cho biết trước ai đang làm việc cho mình và với tư cách gì.",
    portraitPending:
      "Huy hiệu tạm. Sẽ thay bằng ảnh chân dung ông Nikolaos khi khách hàng gửi ảnh.",
    placeholderTeam: "Ảnh chân dung đội ngũ sẽ do khách hàng cung cấp. Tên và bằng cấp đã xác nhận.",
  },

  whyUs: {
    eyebrow: "Vì sao chọn chúng tôi",
    title: "Vì sao nên làm việc với chúng tôi",
    lead: "Nền tảng kinh doanh đa quốc gia của chúng tôi bảo đảm sự thấu hiểu sâu sắc nhu cầu công việc của quý vị.",
    body: [
      "Chúng tôi tôn trọng thời gian của quý vị, làm việc hiệu quả và tuyệt đối trung thành với khoản đầu tư của quý vị. Chúng tôi đứng cạnh quý vị và làm việc cho quý vị cho đến khi công việc hoàn tất.",
    ],
    points: [
      {
        title: "Một đội ngũ, một bộ hồ sơ",
        body: "Visa, ngân hàng, bất động sản, cải tạo và quản lý đều do cùng những người đó lo. Không có việc gì rơi vào khoảng trống giữa hai nhà cung cấp.",
      },
      {
        title: "Chi phí pháp lý do chúng tôi chịu",
        body: "Luật sư làm hồ sơ thẻ vàng cho quý vị được chúng tôi trả tiền, không cộng vào hoá đơn của quý vị ở cuối.",
      },
      {
        title: "Mặc định làm từ xa",
        body: "Tài khoản ngân hàng, giao dịch mua và hồ sơ đều có thể hoàn tất khi quý vị đang ở bất cứ đâu trên thế giới.",
      },
      {
        title: "Kỷ luật đầu tư",
        body: "Lợi suất, tình hình thị trường, quy hoạch và khả năng bán lại đều được đánh giá trước khi bất động sản được đưa ra cho quý vị xem.",
      },
      {
        title: "Năm ngôn ngữ",
        body: "Hồ sơ của quý vị được xử lý bằng tiếng Anh, tiếng Việt, tiếng Hy Lạp, tiếng Latvia và tiếng Thổ Nhĩ Kỳ, bởi những người nói chính ngôn ngữ đó.",
      },
      {
        title: "Có mặt ở bốn thị trường",
        body: "Athens, Thessaloniki, Việt Nam và Thổ Nhĩ Kỳ — văn phòng gần quý vị nhất là văn phòng thật, không phải một hộp thư.",
      },
    ],
  },

  offices: {
    eyebrow: "Chúng tôi ở đâu",
    title: "Văn phòng",
    lead: "Sáu văn phòng trên ba châu lục. Hồ sơ của quý vị được xử lý bằng ngôn ngữ và theo múi giờ thuận tiện cho quý vị.",
    items: [
      {
        slug: "athens",
        city: "Athens",
        country: "Hy Lạp",
        address: "1 Pigasou Street, Kifisia 14564, Hy Lạp",
        phone: "+30 697 300 8000",
        role: "Trụ sở chính · Đội thẻ vàng, pháp lý và tài chính",
        confirmed: true,
      },
      {
        slug: "thessaloniki",
        city: "Thessaloniki",
        country: "Hy Lạp",
        address: "Filippou 28, Perea 57019, Hy Lạp",
        phone: "+49 152 0696 8888",
        role: "Văn phòng NIBELC · Bắc Hy Lạp",
        confirmed: true,
      },
      {
        slug: "budapest",
        city: "Budapest",
        country: "Hungary",
        address: "József krt. 45, 2em.6B, 1085 Budapest, Hungary",
        phone: "",
        role: "NIBELC Manpower Kft · Trung Âu",
        confirmed: true,
      },
      {
        slug: "germany",
        city: "Berlin",
        country: "Đức",
        address: "Dietrichstraße 16, 16356 Ahrensfelde, Đức",
        phone: "",
        role: "NIBELC Germany GmbH · Ahrensfelde",
        confirmed: true,
      },
      {
        slug: "hanoi",
        city: "Hà Nội",
        country: "Việt Nam",
        address: "Tầng 4, toà Rainbow, lô B – CQ1, Văn Quán, Hà Đông, Hà Nội, Việt Nam",
        phone: "",
        role: "Thị trường Việt Nam · tư vấn khách hàng",
        confirmed: true,
      },
      {
        slug: "turkiye",
        city: "Thổ Nhĩ Kỳ",
        country: "Thổ Nhĩ Kỳ",
        address: "",
        phone: "",
        role: "Thị trường Thổ Nhĩ Kỳ · tư vấn khách hàng",
        confirmed: false,
      },
    ],
    pendingLabel: "Địa chỉ đang chờ xác nhận",
    pendingNote: "Trong lúc chờ, xin liên hệ trụ sở chính tại Athens.",
  },

  contact: {
    eyebrow: "Liên hệ",
    title: "Nói chuyện với chuyên viên",
    lead: "Cho chúng tôi biết quý vị đang ở đâu, muốn đạt điều gì và ngân sách dự kiến. Chúng tôi sẽ nói thẳng chương trình có phù hợp không — và hiện đang yêu cầu những gì.",
    form: {
      name: "Họ và tên",
      email: "Email",
      phone: "Điện thoại / WhatsApp",
      country: "Quốc gia đang cư trú",
      interest: "Tôi quan tâm đến",
      interestOptions: [
        "Làm thẻ vàng",
        "Tài khoản ngân hàng tại Hy Lạp",
        "Tư vấn bất động sản",
        "Nghiên cứu bất động sản",
        "Cải tạo",
        "Quản lý bất động sản",
        "Việc khác",
      ],
      budget: "Ngân sách dự kiến",
      budgetOptions: [
        "Chưa quyết định",
        "Dưới 250.000 €",
        "250.000 € – 400.000 €",
        "400.000 € – 800.000 €",
        "Trên 800.000 €",
      ],
      message: "Nội dung",
      consent:
        "Tôi đồng ý cho sử dụng thông tin của mình để trả lời yêu cầu này, như mô tả trong Chính sách quyền riêng tư.",
      submit: "Gửi yêu cầu",
      sending: "Đang gửi…",
      successTitle: "Cảm ơn quý vị — chúng tôi đã nhận được yêu cầu.",
      successBody: "Một chuyên viên sẽ trả lời quý vị trực tiếp, thường trong vòng một ngày làm việc.",
      errorTitle: "Không gửi được tin nhắn của quý vị.",
      errorBody: "Xin thử lại, hoặc gửi email thẳng cho văn phòng Athens của chúng tôi.",
      required: "Bắt buộc",
      invalidEmail: "Xin nhập địa chỉ email hợp lệ",
      consentRequired: "Xin chấp nhận thông báo quyền riêng tư để tiếp tục",
    },
    directTitle: "Liên hệ trực tiếp",
    headOffice: "Trụ sở chính",
    offices: "Văn phòng",
    directions: "Chỉ đường",
    hours: "Thứ Hai đến thứ Sáu, 09:00 – 18:00 (giờ Đông Âu)",
    responseNote: "Chúng tôi trả lời bằng tiếng Anh, tiếng Việt, tiếng Hy Lạp, tiếng Latvia và tiếng Thổ Nhĩ Kỳ.",
  },

  legal: {
    updated: "Cập nhật lần cuối",
    privacy: {
      title: "Chính sách quyền riêng tư",
      lead: "Cách chúng tôi xử lý dữ liệu cá nhân quý vị gửi cho chúng tôi.",
      sections: [
        {
          title: "Chúng tôi là ai",
          body: "NIBELC GROUP – N. KAKKOS ESTATE, Filippou 28, Perea 57019, Hy Lạp, là bên kiểm soát dữ liệu cá nhân thu thập qua website này.",
        },
        {
          title: "Chúng tôi thu thập gì",
          body: "Chỉ những gì quý vị nhập vào biểu mẫu liên hệ: họ tên, địa chỉ email, số điện thoại, quốc gia cư trú, dịch vụ quý vị quan tâm, ngân sách dự kiến và nội dung tin nhắn. Chúng tôi không mua dữ liệu cá nhân và không thu thập từ bên thứ ba.",
        },
        {
          title: "Vì sao chúng tôi dùng",
          body: "Chỉ để trả lời yêu cầu của quý vị và, nếu quý vị trở thành khách hàng, để thực hiện các dịch vụ quý vị thuê chúng tôi. Chúng tôi không bán dữ liệu của quý vị và không dùng cho mục đích tiếp thị không liên quan nếu chưa hỏi ý quý vị trước.",
        },
        {
          title: "Chúng tôi lưu bao lâu",
          body: "Các yêu cầu không phát sinh hợp đồng sẽ được xoá sau 24 tháng. Hồ sơ khách hàng được lưu trong thời hạn mà luật Hy Lạp yêu cầu.",
        },
        {
          title: "Quyền của quý vị",
          body: "Theo GDPR, quý vị có thể yêu cầu truy cập dữ liệu của mình, sửa, xoá, hạn chế xử lý, hoặc nhận một bản sao ở dạng chuyển giao được, và có thể phản đối việc xử lý. Xin gửi thư cho văn phòng Athens của chúng tôi, chúng tôi sẽ trả lời trong vòng một tháng. Quý vị cũng có quyền khiếu nại lên cơ quan bảo vệ dữ liệu tại nước mình.",
        },
        {
          title: "Cookie",
          body: "Trang này không đặt cookie quảng cáo hay phân tích nếu quý vị không chấp nhận rõ ràng. Các cookie thiết yếu giữ cho trang hoạt động và không thể tắt.",
        },
      ],
    },
    terms: {
      title: "Điều khoản sử dụng",
      lead: "Cơ sở để website này được cung cấp.",
      sections: [
        {
          title: "Mục đích của trang",
          body: "Website này giới thiệu dịch vụ của NIBELC GROUP – N. KAKKOS ESTATE. Đây là phần giới thiệu dịch vụ, không phải một lời chào giá, một bản báo giá hay một hợp đồng.",
        },
        {
          title: "Tính chính xác",
          body: "Chúng tôi cố gắng giữ thông tin luôn mới, nhưng quy định nhập cư, mức đầu tư và thời gian xử lý do cơ quan nhà nước quy định và có thể thay đổi mà không báo trước. Không nên dựa vào bất cứ nội dung nào ở đây nếu chưa có xác nhận bằng văn bản của chúng tôi cho trường hợp cụ thể của quý vị.",
        },
        {
          title: "Sở hữu trí tuệ",
          body: "Nội dung chữ, hình ảnh và thiết kế của trang này thuộc về NIBELC GROUP – N. KAKKOS ESTATE hoặc được sử dụng có sự cho phép. Không được sao chép nếu chưa có đồng ý bằng văn bản.",
        },
        {
          title: "Liên kết ngoài",
          body: "Các liên kết tới trang của cơ quan nhà nước hoặc bên thứ ba chỉ mang tính tham khảo. Chúng tôi không chịu trách nhiệm về nội dung của họ.",
        },
        {
          title: "Luật áp dụng",
          body: "Các điều khoản này chịu sự điều chỉnh của luật Hy Lạp.",
        },
      ],
    },
    disclaimer: {
      title: "Tuyên bố miễn trừ",
      lead: "Website này là gì, và không phải là gì.",
      sections: [
        {
          title: "Không phải tư vấn pháp lý hay tư vấn đầu tư",
          body: "Nội dung của website này là thông tin chung. Đây không phải tư vấn pháp lý, tư vấn thuế hay tư vấn đầu tư cá nhân hoá, và không xét đến hoàn cảnh riêng của quý vị.",
        },
        {
          title: "Không bảo đảm kết quả",
          body: "Chúng tôi không và không thể bảo đảm rằng bất kỳ giấy phép cư trú, thị thực hay quốc tịch nào sẽ được cấp. Những quyết định đó hoàn toàn thuộc thẩm quyền của cơ quan chức năng. Các mốc thời gian xử lý chỉ áp dụng khi người nộp đáp ứng các tiêu chí có hiệu lực tại thời điểm đó.",
        },
        {
          title: "Không bảo đảm lợi nhuận",
          body: "Giá trị bất động sản và tiền cho thuê có thể giảm cũng như tăng. Diễn biến quá khứ của thị trường bất động sản Hy Lạp không phải là lời hứa về tương lai, và không có mức lợi nhuận nào được bảo đảm.",
        },
        {
          title: "Mức đầu tư thay đổi",
          body: "Mức đầu tư tối thiểu của chương trình thẻ vàng Hy Lạp đã được điều chỉnh và phụ thuộc vào khu vực cũng như loại bất động sản. Mọi con số báo cho quý vị đều được xác nhận bằng văn bản theo quy định có hiệu lực vào ngày giao dịch.",
        },
      ],
    },
  },

  cookie: {
    title: "Cookie",
    body: "Chúng tôi dùng các cookie thiết yếu để vận hành trang này. Chúng tôi cũng muốn đặt thêm cookie phân tích để hiểu trang được sử dụng ra sao. Quý vị có thể từ chối mà không mất bất kỳ chức năng nào.",
    accept: "Chấp nhận tuỳ chọn",
    decline: "Từ chối tuỳ chọn",
    link: "Chính sách quyền riêng tư",
  },

  footer: {
    tagline: "Dịch vụ thẻ vàng, tư vấn bất động sản và quản lý tài sản tại Hy Lạp.",
    company: "Công ty",
    servicesCol: "Dịch vụ",
    legalCol: "Pháp lý",
    contactCol: "Liên hệ",
    rights: "Bảo lưu mọi quyền.",
    regNote: "Thông tin đăng ký doanh nghiệp sẽ do khách hàng cung cấp.",
    disclaimerShort:
      "Thông tin trên trang này mang tính chung và không phải tư vấn pháp lý hay tư vấn đầu tư. Giấy phép cư trú do cơ quan chức năng Hy Lạp cấp.",
  },

  /**
   * Số liệu lấy từ tài liệu khách gửi: CHƯƠNG TRÌNH VISA ĐẦU TƯ VÀ ĐỊNH CƯ
   * LATVIA.pdf (10 trang). Diện dùng ở đây là GÓP VỐN DOANH NGHIỆP — đúng
   * diện mà Luật Nhập cư mới giữ lại, không phải diện bất động sản hay gửi
   * tiền ngân hàng đang bị đề xuất bỏ. `lawBody` nói rõ chỗ này.
   */
  latvia: {
    eyebrow: "Latvia",
    title: "Định cư Latvia qua đầu tư doanh nghiệp",
    lead: "Chương trình cho phép nhà đầu tư và gia đình có quyền cư trú châu Âu thông qua đầu tư trực tiếp vào doanh nghiệp tại Latvia, kèm lộ trình lên thường trú và quốc tịch nếu đáp ứng điều kiện.",
    photoAlt: [
      "Quảng trường Nhà Hắc Đầu ở Riga",
      "Riga bên sông Daugava lúc hoàng hôn",
      "Mái nhà phố cổ Riga",
      "Kiến trúc kính hiện đại tại Riga",
      "Toàn cảnh Riga",
    ],
    countryTitle: "Đất nước Latvia",
    country: [
      "Nằm ở Bắc Âu, tiếp giáp biển Baltic — cửa ngõ nối các nước Bắc Âu với Đông và Trung Âu.",
      "Dân số khoảng 2 triệu người. Thủ đô là Riga.",
      "Gia nhập Liên minh châu Âu năm 2004 và khối Schengen năm 2007.",
      "Miễn visa đi lại, sinh sống và học tập tại 29 quốc gia thành viên Schengen.",
      "Trung tâm logistics trọng điểm, thuận lợi cho thương mại xuyên biên giới.",
      "Hạ tầng công nghệ hiện đại, tốc độ Internet thuộc nhóm nhanh nhất thế giới.",
    ],
    benefitTitle: "Quyền lợi",
    residencyTitle: "Về cư trú",
    residency: [
      "Quyền cư trú hợp pháp tại Latvia",
      "Có thể sinh sống, học tập và đầu tư tại Latvia",
      "Tự do di chuyển trong khối Schengen",
      "Có thể bảo lãnh người thân trong gia đình",
      "Lộ trình lên thường trú và quốc tịch nếu đáp ứng điều kiện",
    ],
    investTitle: "Về đầu tư",
    invest: [
      "Có thể sở hữu và vận hành doanh nghiệp tại Latvia",
      "Tiếp cận thị trường châu Âu thông qua doanh nghiệp",
      "Nắm giữ cổ phần và nhận cổ tức từ công ty đăng ký tại Latvia",
      "Sinh lời từ đầu tư và tăng trưởng vốn",
    ],
    condTitle: "Điều kiện tham gia",
    basicTitle: "Điều kiện cơ bản",
    basic: [
      "Trên 18 tuổi, không phải công dân EU/EEA/Thụy Sĩ",
      "Lý lịch tư pháp rõ ràng, không tiền án nghiêm trọng",
      "Hộ chiếu hợp lệ và bảo hiểm y tế có hiệu lực tại Schengen",
      "Chứng minh nguồn tiền hợp pháp theo quy định phòng chống rửa tiền",
      "Có khả năng tài chính để thực hiện khoản đầu tư",
    ],
    amountTitle: "Mức đầu tư",
    amount: [
      "Tối thiểu 50.000 € vào vốn điều lệ của một công ty thương mại.",
      "Sau khi có quyết định chấp thuận cấp thị thực lần đầu, nộp thêm 10.000 € vào ngân sách nhà nước.",
      "Hình thức phổ biến: thành lập công ty mới, góp vốn vào doanh nghiệp bản địa, hoặc mua cổ phần doanh nghiệp đang hoạt động.",
    ],
    familyTitle: "Thành viên gia đình đi cùng",
    family: [
      "Vợ hoặc chồng và con dưới 18 tuổi",
      "Con trên 18 tuổi đang đi học và còn phụ thuộc",
    ],
    costTitle: "Tổng khoản đầu tư dự kiến",
    costCols: ["Hạng mục", "Thành tiền (€)"],
    costs: [
      { item: "Đầu tư vốn điều lệ công ty", amount: "55.000" },
      { item: "Phí xin giấy phép cư trú", amount: "5.000" },
      { item: "Chi phí khác — phí nhà nước, bảo hiểm, khám sức khoẻ (ước tính)", amount: "500" },
      { item: "Đăng ký địa chỉ cư trú tại Latvia (hàng năm)", amount: "1.500" },
      { item: "Nộp vào ngân sách nhà nước, sau khi được chấp thuận", amount: "10.000" },
    ],
    costTotalLabel: "Tổng",
    costTotal: "72.000",
    costNotes: [
      "Có thể phát sinh chi phí dịch thuật nếu hồ sơ không phải tiếng Anh, Đức hoặc Pháp.",
      "Một số chi phí nhỏ có thể thay đổi tuỳ từng trường hợp cụ thể.",
      "Số dư cần có trong tài khoản tại Latvia khoảng 84.000 €, gồm vốn đầu tư và chi phí sinh hoạt tối thiểu theo yêu cầu.",
    ],
    processTitle: "Quy trình",
    process: [
      { title: "Chuẩn bị hồ sơ tại Việt Nam", body: "Hộ chiếu còn hạn từ một năm trở lên, ảnh 4×5 nền trắng, phiếu lý lịch tư pháp số 2, giấy tờ đã hợp pháp hoá lãnh sự, giấy tờ chứng minh tài chính." },
      { title: "Chuẩn bị tài chính", body: "Chuẩn bị số dư cần có trong tài khoản tại Latvia, gồm vốn đầu tư và chi phí sinh hoạt tối thiểu theo yêu cầu." },
      { title: "Xử lý hồ sơ tại Latvia", body: "Ký giấy uỷ quyền, mở tài khoản ngân hàng tại Latvia, chuyển tiền vào tài khoản mới lập, nộp hồ sơ tại Cơ quan Di trú OCMA." },
      { title: "Chờ xét duyệt", body: "Phí xét theo tốc độ: 30 ngày 160 €/người, 10 ngày 280 €/người, 5 ngày 560 €/người. Đây là thời gian ra quyết định, chưa chắc đã là kết quả cuối cùng." },
      { title: "Nhận kết quả", body: "Hồ sơ có thể được phê duyệt, bị từ chối, hoặc bị yêu cầu bổ sung — trường hợp yêu cầu bổ sung là phổ biến. Thời gian có thể kéo dài đến 4 tháng, tối đa 12 tháng với trường hợp đặc biệt." },
      { title: "Thực hiện đầu tư", body: "Thành lập hoặc góp vốn vào công ty tại Latvia, hoàn tất thủ tục pháp lý và vận hành, duy trì điều kiện để gia hạn." },
    ],
    lawTitle: "Một điều cần biết về thời điểm này",
    lawBody: "Luật Nhập cư Latvia đang được sửa. Bản Quốc hội thông qua ngày 11/06/2026 bỏ diện bất động sản và diện gửi tiền ngân hàng, nhưng GIỮ diện góp vốn doanh nghiệp — chính là diện của chương trình này. Tổng thống trả luật về xem xét lần hai ngày 19/06 và Quốc hội bàn lại ngày 20/08; đến thời điểm kiểm chứng, luật chưa có hiệu lực nên hồ sơ vẫn xét theo quy định hiện hành. Chúng tôi xác nhận bằng văn bản điều gì áp dụng cho trường hợp của quý vị trước khi quý vị chuyển bất kỳ khoản tiền nào.",
    verified: "Số liệu theo hồ sơ chương trình của NIBELC · kiểm chứng pháp lý ngày 28/08/2026",
    cta: "Hỏi chúng tôi về Latvia",
    note: "Trang này là thông tin chung về chương trình, không phải tư vấn pháp lý hay tư vấn đầu tư. Điều kiện, mức đầu tư và thời gian xử lý do cơ quan chức năng Latvia đặt ra và có thể thay đổi.",
  },

  properties: {
    eyebrow: "Bất động sản",
    title: "Bốn căn hộ tại Peristeri, Athens",
    lead: "Bốn căn hộ đã cải tạo mới hoàn toàn. Dưới đây là toàn bộ hồ sơ chủ đầu tư cung cấp: giới thiệu, giá bán, giá cho thuê tham khảo, sơ đồ mặt bằng và phối cảnh nội thất.",
    linkLabel: "Xem hồ sơ 4 căn hộ",
    backLabel: "Về trang chủ",
    priceLabel: "Giá mua",
    rentLabel: "Cho thuê dài hạn",
    bedsLabel: "Phòng ngủ",
    bathsLabel: "Phòng tắm",
    sizeLabel: "Diện tích",
    sizePending: "Đang xác nhận",
    typeLabel: "Loại hình",
    typeValue: "Căn hộ",
    capacityLabel: "Sức chứa thiết kế",
    introTitle: "Giới thiệu",
    intro: "Căn hộ toạ lạc tại Peristeri, Athens, không gian thiết kế theo hướng ở thực — gọn gàng, dễ sử dụng; phù hợp cho sinh hoạt gia đình nhỏ, cá nhân hoặc cư trú dài hạn. Phong cách nội thất hiện đại, trung tính, dễ cá nhân hoá.",
    planTitle: "Sơ đồ mặt bằng",
    layoutTitle: "Bố cục căn hộ",
    galleryTitle: "Phối cảnh nội thất",
    galleryNote: "Ảnh phối cảnh do chủ đầu tư cung cấp, dùng chung cho cả bốn căn. Sơ đồ mặt bằng thì mỗi căn một bản riêng.",
    cta: "Hỏi hồ sơ chi tiết",
    note: "Giá và tình trạng cập nhật ngày 26/08/2026, có thể thay đổi. Mức đầu tư đủ điều kiện thẻ vàng phụ thuộc khu vực và loại bất động sản — chúng tôi xác nhận bằng văn bản cho từng căn trước khi quý vị cam kết.",
    eligibilityTitle: "Cần biết trước khi cân nhắc",
    eligibility: [
      "Peristeri thuộc vùng Attica. Từ 2024 Attica áp mức đầu tư 800.000 € cho diện mua bất động sản. Giá 250.000–270.000 € của các căn này KHÔNG tự động đủ điều kiện thẻ vàng; chỉ đủ nếu thuộc diện chuyển đổi công trình thương mại sang nhà ở hoặc trùng tu công trình di sản.",
      "Hồ sơ gốc ghi diện tích là “XXX m²” — chưa có số thật. Luật đòi tối thiểu 120 m² cho mức 400.000 € và 800.000 €, nên đây là thông tin bắt buộc phải có trước khi quyết định.",
      "Bất động sản dùng để xin thẻ vàng bị cấm cho thuê ngắn hạn kiểu Airbnb: vi phạm bị thu hồi thẻ và phạt tới 50.000 €. Các căn này chỉ nên cho thuê dài hạn.",
    ],
    commitTitle: "Cam kết đi kèm mỗi căn",
    commitments: [
      "Căn hộ được cải tạo mới 100%",
      "Thuê lại với giá tốt sau khi chủ nhà nhận nhà",
      "Vận hành bền vững và giữ gìn tình trạng căn hộ",
    ],
    items: {
      "apt-1": {
        name: "Căn hộ 1",
        capacity: "khoảng 10 người",
        layout: "Mặt bằng bố trí theo mô hình lưu trú tập thể, khu ngủ trung tâm dùng giường tầng hai bên để tối ưu diện tích. Khu sinh hoạt chung và tủ lưu trữ đặt phía trước, kết nối trực tiếp với bếp mở và bàn ăn dài. Cụm khu vệ sinh – phòng tắm gom về một phía, tách biệt, thuận tiện sử dụng đồng thời. Tổng thể thông thoáng, phân khu rõ ràng.",
      },
      "apt-2": {
        name: "Căn hộ 2",
        capacity: "tối đa 8 người",
        layout: "Căn hộ thiết kế theo mô hình lưu trú nhóm. Khu ngủ tập trung dùng giường tầng, gồm 02 phòng tắm khép kín. Bếp mở kết hợp không gian sinh hoạt chung tạo cảm giác thoáng đãng, thuận tiện cho nấu ăn và giao lưu. Căn hộ có lối đi kết nối trực tiếp với khu vực cầu thang chung, thuận lợi cho việc di chuyển và quản lý.",
      },
      "apt-3": {
        name: "Căn hộ 3",
        capacity: "tối đa 8 người",
        layout: "Căn hộ gộp (Apartment 03 & 04) hợp nhất thành một căn diện tích lớn, phù hợp cho gia đình đông người. Không gian gồm 02 khu ngủ riêng biệt, 02 khu sinh hoạt chung, 02 bếp mở, hệ tủ lưu trữ đầy đủ và nhiều phòng tắm – khu vệ sinh bố trí khoa học, bảo đảm riêng tư cho từng khu chức năng. Căn hộ có hai lối vào (lối chính và lối phụ), bố cục linh hoạt, dễ cải tạo theo nhu cầu.",
      },
      "apt-4": {
        name: "Căn hộ 4",
        capacity: "tối đa 10 người",
        layout: "Căn hộ thiết kế theo mô hình lưu trú nhóm. Không gian gồm khu ngủ tập trung với hệ giường tầng, nhiều phòng tắm và khu vệ sinh riêng biệt, bếp mở kết hợp khu sinh hoạt chung rộng rãi và khu lưu trữ tiện lợi. Bố cục khoa học, tối ưu khả năng sử dụng cho nhiều người cùng lúc.",
      },
    },
  },

  assistant: {
    open: "Hỏi trợ lý",
    close: "Đóng khung chat",
    title: "Trợ lý thẻ vàng",
    subtitle: "Trả lời từ thông tin đã công bố, 24/7",
    greeting:
      "Xin chào. Quý vị có thể hỏi tôi về thẻ vàng Hy Lạp, dịch vụ hoặc văn phòng của chúng tôi. Việc gì liên quan tới hồ sơ riêng thì tôi chuyển sang chuyên viên.",
    placeholder: "Nhập câu hỏi…",
    send: "Gửi",
    thinking: "Đang trả lời…",
    suggestions: [
      "Mức đầu tư hiện nay là bao nhiêu?",
      "Tôi cho thuê Airbnb được không?",
      "Tôi có phải sang Hy Lạp sống không?",
      "Tôi được đưa ai vào hồ sơ?",
    ],
    offlineNote: "— trả lời lấy từ mục Hỏi đáp trên website.",
    noAnswer:
      "Thông tin đã công bố của chúng tôi chưa có nội dung đó. Chuyên viên sẽ trả lời chính xác hơn — quý vị dùng trang liên hệ hoặc gọi văn phòng Athens.",
    humanCta: "Nói chuyện với người thật",
    disclaimer:
      "Trợ lý tự động. Chỉ là thông tin chung, không phải tư vấn pháp lý hay tư vấn đầu tư. Xin đừng gửi số hộ chiếu, tài khoản ngân hàng hay giấy tờ vào khung này.",
    errorMsg: "Có lỗi xảy ra. Xin thử lại hoặc liên hệ trực tiếp với chúng tôi.",
  },

  notFound: {
    title: "Không tìm thấy trang",
    body: "Trang quý vị tìm không tồn tại hoặc đã được chuyển đi.",
    cta: "Về trang chủ",
  },
};
